...
Server Monitoring System v8.62.57
...
def show_main_menu(update, context):
    ...
    if update.message:
        update.message.reply_text(
            text, parse_mode="Markdown", reply_markup=main_menu(extension_manager)
        )
    else:
...
def start_command(update, context):
    ...
    if extension_manager.is_extension_enabled("resource_monitor"):
        keyboard.insert(
            1, [InlineKeyboardButton("📊 Проверить все ресурсы", callback_data="check_resources")]
        )
        keyboard.insert(
            3,
            [
                InlineKeyboardButton(
                    "📈 Ресурсы одного сервера", callback_data="show_resources_menu"
                )
            ],
        )
...
def show_about_bot(update, context):
    ...
    if update.message:
        ...
        update.callback_query.edit_message_text(
            about_text, parse_mode="Markdown", reply_markup=reply_markup
        )
        ...
        update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🛠️ Управление расширениями", callback_data="extensions_menu"
                        )
                    ]
                ]
            ),
    ...
    try:
        ...
        if monitor_server_ip in server_status:
            ...
            for chat_id in config.CHAT_IDS:
                bot.send_message(
                    chat_id=chat_id, text="🔧 Статус сервера мониторинга принудительно исправлен"
                )
        else:
        ...
        for service, host, port in checks:
            try:
                if port:
                    ...
                    message += (
                        f"{status} {service}: {'доступен' if result == 0 else 'недоступен'}\n"
                    )
                else:
...
def diagnose_windows_command(update, context):
    ...
    if wmi_result:
        message += (
            f"• WMI: 🟢 OK (CPU: {wmi_result.get('cpu', 0)}%, RAM: {wmi_result.get('ram', 0)}%)\n"
        )
    else:
        message += "• WMI: 🔴 FAIL\n"
...
def get_handlers():
    ...
    return [
        ...
        CommandHandler(
            "check_single", lambda u, c: show_server_selection_menu(u, c, "check_availability")
        ),
        CommandHandler(
            "check_resources_single",
            lambda u, c: show_server_selection_menu(u, c, "check_resources"),
        ),
...
def get_callback_handlers():
    ...
    return [
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("manual_check")(u, c), pattern="^manual_check$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("monitor_status")(u, c), pattern="^monitor_status$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("servers_list")(u, c), pattern="^servers_list$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("silent_status")(u, c), pattern="^silent_status$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("check_resources")(u, c), pattern="^check_resources$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("control_panel")(u, c), pattern="^control_panel$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("daily_report")(u, c), pattern="^daily_report$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("diagnose_menu")(u, c), pattern="^diagnose_menu$"
        ),
        CallbackQueryHandler(lambda u, c: lazy_handler("close")(u, c), pattern="^close$"),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("full_report")(u, c), pattern="^full_report$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("force_silent")(u, c), pattern="^force_silent$"
        ),
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("toggle_silent")(u, c), pattern="^toggle_silent$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("resource_history")(u, c), pattern="^resource_history$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("debug_report")(u, c), pattern="^debug_report$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("monitor_main")(u, c), pattern="^monitor_main$"
        ),
        CallbackQueryHandler(lambda u, c: lazy_handler("main_menu")(u, c), pattern="^main_menu$"),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("toggle_monitoring")(u, c), pattern="^toggle_monitoring$"
        ),
        CallbackQueryHandler(lambda u, c: lazy_handler("close")(u, c), pattern="^close$"),
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("db_backups_today")(u, c), pattern="^db_backups_today$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("db_backups_summary")(u, c), pattern="^db_backups_summary$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("db_backups_detailed")(u, c), pattern="^db_backups_detailed$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("db_backups_list")(u, c), pattern="^db_backups_list$"
        ),
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("resource_page")(u, c), pattern="^resource_page_"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("refresh_resources")(u, c), pattern="^refresh_resources$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("close_resources")(u, c), pattern="^close_resources$"
        ),
        # Обработчики для раздельной проверки по типам серверов
        CallbackQueryHandler(
            lambda u, c: lazy_handler("check_linux")(u, c), pattern="^check_linux$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("check_windows")(u, c), pattern="^check_windows$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("check_other")(u, c), pattern="^check_other$"
        ),
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_hosts")(u, c), pattern="^backup_hosts$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_refresh")(u, c), pattern="^backup_refresh$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_host_")(u, c), pattern="^backup_host_"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("db_backups_today")(u, c), pattern="^db_backups_today$"
        ),
        ...
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_main")(u, c), pattern="^backup_main$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_proxmox")(u, c), pattern="^backup_proxmox$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_proxmox_menu")(u, c), pattern="^backup_proxmox_menu$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_proxmox_patterns")(u, c),
            pattern="^backup_proxmox_patterns$",
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_databases")(u, c), pattern="^backup_databases$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_mail")(u, c), pattern="^backup_mail$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_host_")(u, c), pattern="^backup_host_"
        ),
        CallbackQueryHandler(lambda u, c: lazy_handler("db_detail_")(u, c), pattern="^db_detail_"),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("backup_stale_hosts")(u, c), pattern="^backup_stale_hosts$"
        ),
        # Обработчики расширений
        CallbackQueryHandler(
            lambda u, c: lazy_handler("extensions_menu")(u, c), pattern="^extensions_menu$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("extensions_refresh")(u, c), pattern="^extensions_refresh$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("ext_enable_all")(u, c), pattern="^ext_enable_all$"
        ),
        CallbackQueryHandler(
            lambda u, c: lazy_handler("ext_disable_all")(u, c), pattern="^ext_disable_all$"
        ),
        CallbackQueryHandler(
            lambda u, c: extensions_callback_handler(u, c), pattern="^ext_toggle_"
        ),
        ...
        CallbackQueryHandler(
            lambda u, c: show_server_selection_menu(u, c, "check_availability"),
            pattern="^show_availability_menu$",
        ),
        CallbackQueryHandler(
            lambda u, c: show_server_selection_menu(u, c, "check_resources"),
            pattern="^show_resources_menu$",
        ),
        CallbackQueryHandler(
            lambda u, c: handle_single_check(
                u, c, u.callback_query.data.replace("check_availability_", "")
            ),
            pattern="^check_availability_",
        ),
        CallbackQueryHandler(
            lambda u, c: handle_single_resources(
                u, c, u.callback_query.data.replace("check_resources_", "")
            ),
            pattern="^check_resources_",
        ),
...
def show_server_selection_menu(update, context, action="check_availability"):
    ...
    if action == "check_resources" and not extension_manager.is_extension_enabled(
        "resource_monitor"
    ):
...
def handle_single_check(update, context, server_id):
    ...
    if server:
        row_actions = [
            InlineKeyboardButton(
                "🔄 Проверить снова", callback_data=f"check_availability_{server['ip']}"
            )
        ]
        if extension_manager.is_extension_enabled("resource_monitor"):
            row_actions.insert(
                0,
                InlineKeyboardButton(
                    "📊 Проверить ресурсы", callback_data=f"check_resources_{server['ip']}"
                ),
            )
...
def handle_single_resources(update, context, server_id):
    ...
    if server:
        keyboard.append(
            [
                InlineKeyboardButton(
                    "📡 Проверить доступность", callback_data=f"check_availability_{server['ip']}"
                ),
                InlineKeyboardButton(
                    "🔄 Обновить ресурсы", callback_data=f"check_resources_{server['ip']}"
                ),
            ]
        )
...
def add_quick_check_buttons(keyboard, server_ip=None):
    ...
    if server_ip:
        ...
        row_actions = [
            InlineKeyboardButton(
                "🔍 Проверить доступность", callback_data=f"check_availability_{server_ip}"
            )
        ]
        if extension_manager.is_extension_enabled("resource_monitor"):
            row_actions.append(
...
def create_quick_actions_menu(server_ip):
    ...
    keyboard.extend(
        [
            [
                InlineKeyboardButton(
                    "📋 Информация о сервере", callback_data=f"server_info_{server_ip}"
                )
            ],
            [
                InlineKeyboardButton(
                    "🔄 Проверить снова", callback_data=f"check_availability_{server_ip}"
                )
            ],
            [
                InlineKeyboardButton("🏠 На главную", callback_data="main_menu"),
