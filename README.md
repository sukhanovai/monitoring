...
### Разруливание конфликтов PR-ветки с `develop` из CLI

Если GitHub показывает конфликт между вашей PR-веткой и `develop`, можно запустить helper-скрипт (PowerShell):

```powershell
./scripts/resolve_merge_conflicts.ps1
```

Скрипт делает `fetch`, `merge` и пытается автоматически закрыть типовые конфликтные файлы с версиями.
