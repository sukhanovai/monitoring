...
python main.py --check targeted_checks --server 192.168.3.470 --mode resources
```
...
В репозитории добавлен Android-проект `android-client/` (Kotlin + Compose), который работает с BFF API по адресу `https://api.202020.ru:8443` и не ломает существующий Telegram-канал управления. В клиенте есть вкладки: статус серверов, control actions, просмотр Proxmox-бэкапов и изменение monitoring-настроек.
