...
### Fixed
- RU: Вторая причина того же симптома, что и в 8.65.1 (там схлопывались алиасные ключи категорий вида `barnaul_backups`). Утренний и ручной отчёты показывали «🔴 Барнаул: 0/3 (0%)» и «Нет бэкапов за 24ч — Барнаул: 7.7, DOC, bases», хотя бэкапы приходили каждую ночь и в деталях по каждой базе («📋 Детали по 7.7» и др.) ошибок не было. Сводки бэкапов БД знали только четыре встроенные категории, а базы пользовательской категории (созданной через «🗂️ Управление категориями», например «Филиалы») подставлялись в категорию `barnaul`: парсер писем сохраняет такой бэкап с `backup_type`, равным имени категории, поэтому ни одна запись в `backups.db` не совпадала с ожидаемой парой (`barnaul`, имя базы) и все базы категории считались пропавшими. Теперь сводки перебирают все категории конфигурации и показывают пользовательскую категорию под её собственным именем — так же, как их всегда показывало меню «🗃️ Бэкапы баз данных» (`extensions/backup_monitor/backup_utils.py`: `build_config_databases`, `normalize_config_backup_type`, `get_category_display_name`, `order_categories`; применено в `get_database_backup_stats`, `get_backup_summary` и `core/monitor_parts/report.py`).
- EN: A second cause of the same symptom as in 8.65.1 (which collapsed alias category keys such as `barnaul_backups`). The morning and manual reports showed "🔴 Барнаул: 0/3 (0%)" and "no backups in 24h — Барнаул: 7.7, DOC, bases" even though the backups arrived every night and the per-database details ("📋 Детали по 7.7" and others) reported no errors. The database backup summaries knew only the four built-in categories, and databases of a user-defined category (created via "🗂️ Manage categories", e.g. "Филиалы") were folded into the `barnaul` category: the mail parser stores such a backup with `backup_type` equal to the category name, so no row in `backups.db` matched the expected (`barnaul`, database) pair and every database of the category was counted as missing. The summaries now iterate over all configured categories and show a user-defined category under its own name — exactly as the "🗃️ Database backups" menu always did (`extensions/backup_monitor/backup_utils.py`: `build_config_databases`, `normalize_config_backup_type`, `get_category_display_name`, `order_categories`; applied in `get_database_backup_stats`, `get_backup_summary` and `core/monitor_parts/report.py`).
- RU: Категория конфигурации `company_databases` больше не теряется в меню «🗃️ Бэкапы баз данных»: таблица соответствий «категория конфига → `backup_type`» стала общей для меню и отчётов (`extensions/backup_monitor/backup_handlers.py`: `_normalize_config_backup_type` делегирует в `backup_utils`), раньше в ней не было формы во множественном числе.
- EN: The `company_databases` configuration category is no longer lost in the "🗃️ Database backups" menu: the "config category → `backup_type`" mapping is now shared between the menu and the reports (`extensions/backup_monitor/backup_handlers.py`: `_normalize_config_backup_type` delegates to `backup_utils`); it previously lacked the plural spelling.

### Added
- RU: Тесты сводки по пользовательской категории БД: `tests/test_backup_db_stats.py` (категория «Филиалы» считается под своим именем, в сводке нет ложного «Нет бэкапов»).
- EN: Tests for the summary of a user-defined database category: `tests/test_backup_db_stats.py` (the "Филиалы" category is counted under its own name and the summary carries no false "no backups" alarm).

## [8.65.1] - 2026-08-27

### Fixed
- RU: Секция «🗃️ Бэкапы БД» утреннего/ручного отчёта показывала филиалы Барнаула как «0/3 (0%)» и «Нет бэкапов за 24ч», хотя бэкапы были свежими и успешными (интерактивные детали по той же базе их находили). Мастер «⚙️ Настройка паттернов» предлагает категории прямо из `DATABASE_CONFIG` — «полными» ключами (`barnaul_backups`, `client_databases`, ...), а не короткими (`barnaul`, `client`, ...), которые ищет `parse_database_backup`. Паттерн, сохранённый под таким ключом, распознавал письмо через запасную ветку «пользовательских категорий» и писал `backup_type='barnaul_backups'` вместо `'barnaul'` — сводка сравнивала по литералу `'barnaul'` и не находила такие (успешные, свежие) бэкапы, а интерактивные детали «выживали» только за счёт отдельного фолбэка через `_get_latest_backup_type`, которого в сводке не было. Теперь `get_database_patterns_from_config` схлопывает такие алиасы в канонические категории до разбора писем (`modules/mail_parts/patterns.py`), а `get_database_backup_stats`/сводка бэкапов БД и меню «🗃️ Бэкапы баз данных» распознают уже накопленные записи с алиасным `backup_type` (`extensions/backup_monitor/backup_utils.py`: `_canonical_backup_category`; `extensions/backup_monitor/backup_handlers.py`).
- EN: The "🗃️ DB backups" section of the morning/manual report showed the Barnaul branches as "0/3 (0%)" and "No backups in 24h" even though the backups were recent and successful (the interactive per-database details found them fine). The "⚙️ Настройка паттернов" wizard offers categories straight from `DATABASE_CONFIG` — "full" keys (`barnaul_backups`, `client_databases`, ...) rather than the short ones (`barnaul`, `client`, ...) that `parse_database_backup` looks for. A pattern saved under such a key matched incoming mail through the "custom category" fallback branch and wrote `backup_type='barnaul_backups'` instead of `'barnaul'` — the summary compared against the literal `'barnaul'` and never found those (successful, fresh) backups, while the interactive detail view only "survived" thanks to a separate fallback through `_get_latest_backup_type` that the summary lacked. `get_database_patterns_from_config` now collapses such aliases into the canonical category before parsing mail (`modules/mail_parts/patterns.py`), and the DB backup summary plus the "🗃️ Database backups" menu now recognize already-stored rows with an aliased `backup_type` (`extensions/backup_monitor/backup_utils.py`: `_canonical_backup_category`; `extensions/backup_monitor/backup_handlers.py`).

### Added
- RU: Регрессионный тест воспроизводит ровно этот случай — бэкапы с `backup_type='barnaul_backups'` больше не считаются отсутствующими в сводке за 24ч (`tests/test_backup_db_stats.py`).
- EN: A regression test reproduces this exact case — backups stored with `backup_type='barnaul_backups'` are no longer reported missing in the 24h summary (`tests/test_backup_db_stats.py`).

## [8.65.0] - 2026-08-12

### Added
- RU: Многопользовательский режим в Android-клиенте. Новый раздел `⚙️ Настройки` → `🔔 Мои оповещения`: получать ли утренний/сводный отчёт и оповещения мониторинга, уровни (критические/предупреждения/информационные), подписка на категории с кнопками «Все»/«Очистить», личные тихие часы. Настройки личные — сбор данных остаётся общим для всех (`android-client/.../MyNotificationsSection.kt`, методы `setMyReportsEnabled`, `setMyAlertsEnabled`, `toggleMyAlertLevel`, `toggleMyAlertCategory`, `setMyQuietHoursEnabled` в `MainViewModel`).
- EN: Multi-user mode in the Android client. A new `⚙️ Settings` → `🔔 My alerts` section: whether to receive the morning/summary report and monitoring alerts, severity levels (critical/warning/info), category subscriptions with "All"/"Clear" buttons, and personal quiet hours. The settings are personal — data collection stays shared (`android-client/.../MyNotificationsSection.kt`, `setMyReportsEnabled`, `setMyAlertsEnabled`, `toggleMyAlertLevel`, `toggleMyAlertCategory`, `setMyQuietHoursEnabled` in `MainViewModel`).
- RU: Карточка «📱 Это устройство» с `device_id`, списком пользователей реестра и привязкой телефона: можно привязать устройство к существующему пользователю или создать нового прямо с телефона (логин `android_<8 символов device_id>`). Приложение и раньше отправляло `device_id` при обмене токена, поэтому сразу после привязки сервер отдаёт этому телефону персональные отчёт и оповещения (`GET /v1/me`, `PATCH /v1/me/notifications`, `GET|POST /v1/users`, `POST /v1/users/<id>/channels` в `MonitoringApi`).
- EN: A "📱 This device" card with the `device_id`, the registry user list and device linking: the phone can be linked to an existing user or a new one created right from the device (login `android_<8 chars of device_id>`). The app already sent `device_id` during token exchange, so right after linking the server serves this phone its personal report and alerts (`GET /v1/me`, `PATCH /v1/me/notifications`, `GET|POST /v1/users`, `POST /v1/users/<id>/channels` in `MonitoringApi`).
- RU: Экран «🗒️ Состав отчёта» в приложении подписан владельцем — видно, правится личный состав конкретного пользователя или общесистемный (модель `SettingsReportData` разбирает поля `scope` и `user`, которые сервер отдаёт с 8.64.0).
- EN: The "🗒️ Report composition" screen in the app now shows its owner — whether a specific user's personal composition or the system-wide one is being edited (the `SettingsReportData` model now parses the `scope` and `user` fields the server has returned since 8.64.0).
- RU: Тесты контракта HTTP-API, на который опираются Moshi-модели клиента: `tests/test_mobile_users_api.py` (опознание устройства по `device_id`, частичный `PATCH` личных настроек, запрет персонализации настроек сбора, список пользователей и привязка канала, `scope`/`user` в составе отчёта). Flask в dev-зависимостях CI нет — тест пропускается при его отсутствии. Раздел «Android-клиент» добавлен в `docs/multi_user.md`.
- RU: Подпись владельца в экране «Состав отчёта» очищается, когда состав снова становится общесистемным. Elvis-фолбэк оставлял в состоянии прежнего владельца при `user: null` в ответе, и после отвязки устройства администратором экран продолжал утверждать, что правится личный состав того пользователя, хотя переключатели уже меняли общесистемный. Прежнее значение теперь сохраняется только когда запрос не прошёл вовсе (`android-client/.../MainViewModel.kt`: `reportSettingsData`, `updatedSettings`).
- EN: The owner caption on the "Report composition" screen is now cleared when the composition falls back to system-wide. The Elvis fallback kept the previous owner in state when the response carried `user: null`, so after an admin unlinked the device the screen kept claiming that user's personal composition was being edited while the toggles already updated the global one. The previous value is now kept only when the request failed entirely (`android-client/.../MainViewModel.kt`: `reportSettingsData`, `updatedSettings`).
- EN: Tests for the HTTP API contract the client's Moshi models rely on: `tests/test_mobile_users_api.py` (device lookup by `device_id`, partial `PATCH` of personal settings, refusal to personalize collection settings, user list and channel linking, `scope`/`user` in the report composition). Flask is not in the CI dev dependencies, so the test is skipped when it is missing. An "Android client" section was added to `docs/multi_user.md`.
...
### Fixed
- RU: Кнопка «➕ Привязать этот чат» в меню «👥 Пользователи» отбирала права администратора у нажавшего. Канал принадлежит ровно одному пользователю, поэтому привязка молча уводила текущий чат у прежнего владельца: администратор, нажавший кнопку в своём чате, оказывался обычным пользователем `tg_<chat_id>`, а мигрировавший `admin` оставался без каналов связи — в меню «👥 Пользователи» вместо списка появлялось «⛔ Управление пользователями доступно только администратору», и починить это из бота было нельзя. Теперь кнопка в списке НЕ переназначает уже привязанный чат (открывается карточка владельца с пояснением), а перенос делается осознанно кнопкой «🔗 Привязать этот чат» в карточке нужного пользователя. Создаваемый из чата пользователь наследует роль администратора, если чат перечислен в общем `CHAT_IDS` (`bot/handlers/settings_handlers/users.py`: `link_current_chat_handler`).
- EN: The "➕ Link this chat" button in the "👥 Users" menu stripped the admin rights of whoever pressed it. A channel belongs to exactly one user, so linking silently moved the current chat away from its previous owner: an admin pressing the button in their own chat became a plain `tg_<chat_id>` user, while the migrated `admin` was left without channels — the "👥 Users" menu showed "⛔ User management is available to administrators only" instead of the list, with no way to fix it from the bot. The list button no longer reassigns an already-linked chat (it opens the owner's card with an explanation); moving a chat is now a deliberate action via "🔗 Link this chat" in the target user's card. A user created from a chat inherits the admin role when that chat is listed in the shared `CHAT_IDS` (`bot/handlers/settings_handlers/users.py`: `link_current_chat_handler`).
- RU: Доступ к управлению реестром больше не может быть потерян. Меню открыто, если чат принадлежит администратору, ЛИБО перечислен в общем `CHAT_IDS` (такие чаты и до многопользовательского режима имели полный доступ к настройкам), ЛИБО не привязан ни к кому, ЛИБО в реестре не осталось администратора с каналом связи — аварийный выход (`core/users.py`: `is_admin_telegram_chat`, `can_manage_users`, `has_reachable_admin`). Та же логика применяется к Matrix-команде `!users` (`lib/matrix_commands.py`).
- EN: Access to registry management can no longer be lost. The menu opens if the chat belongs to an admin, OR is listed in the shared `CHAT_IDS` (such chats had full settings access even before multi-user mode), OR is not linked to anyone, OR no admin with a communication channel is left in the registry — a break-glass path (`core/users.py`: `is_admin_telegram_chat`, `can_manage_users`, `has_reachable_admin`). The same logic now guards the Matrix `!users` command (`lib/matrix_commands.py`).
- RU: Последнего администратора нельзя разжаловать, выключить или удалить — `update_user`/`delete_user` возвращают `UserRegistryError` с подсказкой сначала назначить администратором кого-то ещё; в Telegram текст ошибки показывается всплывающим предупреждением, а не теряется в логе (`core/users.py`, `bot/handlers/settings_handlers/users.py`).
- RU: Аварийный выход (пункт 4 правил доступа) больше не открывается по отдельному каналу. `can_manage_users` проверял достижимость администратора на том транспорте, откуда пришла команда, поэтому обычный пользователь Matrix получал право добавлять и удалять пользователей, пока администратор был доступен только в Telegram (и симметрично для Telegram). Теперь достижимость считается по всем каналам сразу (`core/users.py`: `can_manage_users` без параметра `channel_type`, `lib/matrix_commands.py`).
- EN: The break-glass path (rule 4 of the access rules) no longer opens per channel. `can_manage_users` checked admin reachability on the transport the command arrived from, so an ordinary Matrix user could add and delete users while the admin was only reachable via Telegram (and symmetrically for Telegram). Reachability is now evaluated across all channels at once (`core/users.py`: `can_manage_users` without the `channel_type` argument, `lib/matrix_commands.py`).
- EN: The last administrator can no longer be demoted, disabled or deleted — `update_user`/`delete_user` raise `UserRegistryError` suggesting to promote someone else first; in Telegram the error text is shown as a popup alert instead of being swallowed by the log (`core/users.py`, `bot/handlers/settings_handlers/users.py`).

### Added
- RU: В меню «👥 Пользователи» видно, кому принадлежит текущий чат и есть ли он в общем `CHAT_IDS` — без этой строки было непонятно, от чьего имени работают «Мои оповещения» и состав отчёта. Раздел «Права на управление реестром» с порядком восстановления доступа добавлен в `docs/multi_user.md`. Тесты на защиту от потери администратора: `tests/test_users_registry.py`.
- EN: The "👥 Users" menu now shows which user owns the current chat and whether it is listed in the shared `CHAT_IDS` — without that line it was unclear whose "My alerts" and report composition were being edited. A "Registry management rights" section with the recovery procedure was added to `docs/multi_user.md`. Tests covering the lockout protection: `tests/test_users_registry.py`.

## [8.64.0] - 2026-08-10

### Added
- RU: Многопользовательский режим. Появился реестр пользователей (`core/users.py`: таблицы `users`, `user_channels`, `user_preferences` в `data/settings.db`), где у каждого пользователя свои каналы обмена с ботом (`telegram` — chat_id, `matrix` — room_id или MXID, `mobile` — device_id/subject токена, `web` — логин) и свои персональные настройки доставки: состав утреннего/ручного отчёта, получать ли отчёт и оповещения, уровни (`critical`/`warning`/`info`), подписка на категории (`availability`, `resources`, `system`, id расширений) и личные тихие часы. Общие настройки сбора данных (серверы, интервалы, паттерны писем, учётные данные, включённость расширений) остаются едиными для всей системы и персонализации не подлежат — попытка сохранить их персонально завершается `UserRegistryError`. Значение из таблицы `settings` работает как общесистемный дефолт: пользователь получает его, пока не задал своё.
- EN: Multi-user mode. A user registry was added (`core/users.py`: `users`, `user_channels`, `user_preferences` tables in `data/settings.db`), where every user has their own bot channels (`telegram` — chat_id, `matrix` — room_id or MXID, `mobile` — device_id/token subject, `web` — login) and their own delivery preferences: morning/manual report composition, whether to receive the report and alerts, severity levels (`critical`/`warning`/`info`), category subscriptions (`availability`, `resources`, `system`, extension ids) and personal quiet hours. Shared data-collection settings (servers, intervals, mail patterns, credentials, extension toggles) stay system-wide and cannot be personalized — storing them per user raises `UserRegistryError`. The value in the `settings` table acts as the system-wide default until a user sets their own.
- RU: Персональные отчёты (`modules/morning_report.py`). Данные мониторинга собираются один раз на всех, после чего для каждого получателя строится отчёт по ЕГО составу. Секции расширений кэшируются между получателями (`build_report(section_cache=...)`), поэтому тяжёлые live-секции (TLS-сертификаты, ресурсы серверов, свободное место ZFS) опрашивают серверы один раз, а не на каждого пользователя. `force_report`/`force_report_formats`/`generate_report_message` принимают `user_id`.
- EN: Personalized reports (`modules/morning_report.py`). Monitoring data is collected once for everyone, then each recipient's report is built from THEIR composition. Extension sections are cached across recipients (`build_report(section_cache=...)`), so heavy live sections (TLS certificates, server resources, ZFS free space) poll servers once instead of once per user. `force_report`/`force_report_formats`/`generate_report_message` accept `user_id`.
- RU: Персональная маршрутизация оповещений (`lib/alerts.py`). `send_alert` получает адресатов из реестра и раскладывает сообщение по личным каналам с учётом персональных фильтров; добавлены параметры `category` (категория оповещения) и `user_ids` (явные получатели), а также `send_personal_message` для доставки готового сообщения одному пользователю. Категории проставлены на местах вызова: доступность серверов (`core/monitor.py`, `core/monitor_parts/availability.py`, `modules/availability.py`), ресурсы (`core/monitor_parts/alerts.py`), системные сообщения (`core/monitor_parts/lifecycle.py`), TLS и свободное место ZFS (`core/monitor_parts/lifecycle.py` через `functools.partial`).
- EN: Per-user alert routing (`lib/alerts.py`). `send_alert` resolves recipients from the registry and delivers to their personal channels honouring their filters; new `category` and `user_ids` parameters were added, plus `send_personal_message` for delivering a prepared message to a single user. Categories were wired at the call sites: server availability (`core/monitor.py`, `core/monitor_parts/availability.py`, `modules/availability.py`), resources (`core/monitor_parts/alerts.py`), system messages (`core/monitor_parts/lifecycle.py`), TLS and ZFS free space (`core/monitor_parts/lifecycle.py` via `functools.partial`).
- RU: UI. Telegram — меню `🔔 Мои оповещения` и `👥 Пользователи` (`bot/handlers/settings_handlers/users.py`), кнопка «➕ Привязать этот чат» создаёт пользователя и привязывает текущий чат; меню «🗒️ Состав отчёта» теперь правит личный состав владельца чата. Matrix — команды `!me` (личные настройки и свои каналы) и `!users add|link|unlink|enable|disable|delete` (админ), `!report`/`!report config` работают с личным составом. API — `GET /v1/me`, `PATCH /v1/me/notifications`, CRUD `/v1/users` и `/v1/users/<id>/channels`; `GET|PATCH /v1/settings/report` и `action=send_morning_report` персонализированы по владельцу токена (поиск по `device_id`, затем subject/логину).
- EN: UI. Telegram — new `🔔 My alerts` and `👥 Users` menus (`bot/handlers/settings_handlers/users.py`), the "➕ Link this chat" button creates a user and links the current chat; the "🗒️ Report composition" menu now edits the chat owner's personal composition. Matrix — new `!me` (personal settings and own channels) and `!users add|link|unlink|enable|disable|delete` (admin) commands, `!report`/`!report config` operate on the personal composition. API — `GET /v1/me`, `PATCH /v1/me/notifications`, CRUD for `/v1/users` and `/v1/users/<id>/channels`; `GET|PATCH /v1/settings/report` and `action=send_morning_report` are personalized by the token owner (looked up by `device_id`, then token subject/web login).
- RU: Документация `docs/multi_user.md` (модель данных, категории оповещений, команды и API) и раздел «👥 Пользователи и персональные отчёты» в README. Тесты: `tests/test_users_registry.py` (реестр, каналы, персональные настройки, фильтры доставки, миграция) и `tests/test_report_personalization.py` (состав отчёта по пользователю, кэш секций, раскладка оповещений по каналам).
- EN: Documentation `docs/multi_user.md` (data model, alert categories, commands and API) and a "👥 Users and personalized reports" section in the README. Tests: `tests/test_users_registry.py` (registry, channels, personal preferences, delivery filters, migration) and `tests/test_report_personalization.py` (per-user report composition, section cache, alert channel fan-out).

### Changed
- RU: Доступ к Telegram-боту проверяется сначала по реестру (чат привязан к включённому пользователю), затем по прежнему списку `CHAT_IDS` (`bot/handlers/base.py`, `core/monitor_parts/telegram_handlers.py`: общий хелпер `_chat_allowed` вместо 11 копий проверки `str(chat_id) not in config.CHAT_IDS`) — обновление никого не отрезает от бота.
- EN: Telegram bot access is checked against the registry first (the chat belongs to an enabled user) and then against the previous `CHAT_IDS` list (`bot/handlers/base.py`, `core/monitor_parts/telegram_handlers.py`: a shared `_chat_allowed` helper replaces 11 copies of the `str(chat_id) not in config.CHAT_IDS` check) — the upgrade cannot lock anyone out.
- RU: `bot/handlers/commands.py:send_alert` больше не рассылает сообщение прямым циклом по `CHAT_IDS` в обход персональных подписок, а делегирует его `core.monitor_core.send_alert` → `lib.alerts.send_alert`.
- EN: `bot/handlers/commands.py:send_alert` no longer broadcasts via a direct `CHAT_IDS` loop that bypassed personal subscriptions; it delegates to `core.monitor_core.send_alert` → `lib.alerts.send_alert`.
- RU: Лимит in-memory реестра payload'ов отчёта (кнопки разворота секций в Telegram) поднят с 8 до 64: при персональной рассылке каждая отправка регистрирует payload на каждого получателя, и прежнего лимита хватало бы на одного пользователя (`modules/morning_report.py`: `_REPORT_REGISTRY_LIMIT`).
- EN: The in-memory report payload registry limit (Telegram section-expand buttons) was raised from 8 to 64: with personalized delivery every send registers one payload per recipient, so the previous limit would only have covered a single user (`modules/morning_report.py`: `_REPORT_REGISTRY_LIMIT`).

### Migration
- RU: Ручных действий не требуется. При первом обращении к реестру, если он пуст, создаётся пользователь `admin` с ролью администратора: к нему привязываются все чаты из `CHAT_IDS`, `MATRIX_ROOM_ID` и `WEB_LOGIN`, а текущий `REPORT_EXTENSIONS` становится его личным составом отчёта. Пока пользователь один, поведение системы не меняется; при пустом или недоступном реестре доставка автоматически возвращается к прежнему режиму (общий `CHAT_IDS` и общая Matrix-комната).
- EN: No manual steps required. On first access, if the registry is empty, an `admin` user is created: every chat from `CHAT_IDS`, plus `MATRIX_ROOM_ID` and `WEB_LOGIN`, is linked to it, and the current `REPORT_EXTENSIONS` becomes its personal report composition. While there is a single user the behaviour is unchanged; if the registry is empty or unavailable, delivery automatically falls back to the previous mode (shared `CHAT_IDS` and shared Matrix room).

## [8.63.41] - 2026-08-05

### Fixed
- RU: Письма с остатками поставщиков больше не падают с «❌ Ошибка парсинга файла …: can't compare offset-naive and offset-aware datetimes» и «⚠️ Не удалось обработать письмо». Очистка архива остатков сравнивала дату письма (`Date:`, с таймзоной) с `datetime.fromtimestamp(st_mtime)` (без таймзоны); исключение из `cleanup_supplier_stock_archives` всплывало в `parse_email_file`, письмо считалось необработанным и разбиралось на каждом цикле заново. Дата письма приводится к наивному локальному времени (`extensions/supplier_stock_files.py`: `_as_naive_local`, `cleanup_supplier_stock_archives`, `process_supplier_stock_file`).
- EN: Supplier stock emails no longer fail with "❌ Ошибка парсинга файла …: can't compare offset-naive and offset-aware datetimes" and "⚠️ Не удалось обработать письмо". The stock archive cleanup compared the email date (`Date:`, timezone-aware) with `datetime.fromtimestamp(st_mtime)` (naive); the exception escaped `cleanup_supplier_stock_archives` into `parse_email_file`, so the email was treated as unprocessed and re-parsed on every cycle. The email date is now converted to naive local time (`extensions/supplier_stock_files.py`: `_as_naive_local`, `cleanup_supplier_stock_archives`, `process_supplier_stock_file`).
- RU: Смена шаблона вложения у правила почты (например `asd_orig.xls` → `asd_orig.xlsx`, когда поставщик поменял формат) теперь доходит до привязанного правила обработки. Правило, привязанное к источнику, не даёт править «Файл источника» вручную («Название и файл источника берутся из настроек источника»), но само значение подтягивалось только при открытии и сохранении правила в боте — в конфигурации оставалось старое имя, и в логе было «🧩 Остатки поставщиков: подходящие правила обработки не найдены для asd_orig.xlsx», файл выгружался без обработки. Теперь `source_file` синхронизируется с `output_template`/`output_name` источника при каждом чтении и сохранении конфигурации (`extensions/supplier_stock_files.py`: `_sync_processing_rules_with_sources` в `normalize_supplier_stock_config`).
- EN: Changing the attachment template of a mail rule (e.g. `asd_orig.xls` → `asd_orig.xlsx` after the supplier switched formats) now propagates to the linked processing rule. A source-bound rule does not allow editing "Файл источника" by hand ("the name and source file come from the source settings"), but the value was only pulled in when the rule was opened and saved in the bot — the config kept the old name, the log said "🧩 Остатки поставщиков: подходящие правила обработки не найдены для asd_orig.xlsx", and the file was uploaded unprocessed. `source_file` is now synced with the source's `output_template`/`output_name` on every config read and save (`extensions/supplier_stock_files.py`: `_sync_processing_rules_with_sources` in `normalize_supplier_stock_config`).
- RU: Исправлены «❌ Ошибка извлечения паттернов передачи на NAS: 'str' object has no attribute 'get'» и «❌ Ошибка извлечения паттернов конфигов/историй: …» в логе mail_monitor. Если у настройки `BACKUP_PATTERNS` в таблице `settings` проставлен `data_type='string'`, она приходит сырой JSON-строкой, и fallback `BACKUP_PATTERNS.get(...)` падал — письма о передаче на NAS и о бэкапе конфигов/историй молча переставали разбираться (у паттернов БД, снэпшотов и остатков разбор строки уже был). Теперь JSON-настройки приводятся к типу значения по умолчанию при чтении (`config/db_settings.py`: `_coerce_json_setting`), а сборщики паттернов используют общий разбор источника (`modules/mail_parts/patterns.py`: `_as_pattern_mapping`, `_get_subject_patterns`).
- EN: Fixed "❌ Ошибка извлечения паттернов передачи на NAS: 'str' object has no attribute 'get'" and "❌ Ошибка извлечения паттернов конфигов/историй: …" in the mail_monitor log. When the `BACKUP_PATTERNS` setting has `data_type='string'` in the `settings` table, it comes back as a raw JSON string and the `BACKUP_PATTERNS.get(...)` fallback crashed — NAS transfer and config/console backup emails silently stopped being parsed (DB, snapshot and stock patterns already handled the string case). JSON settings are now coerced to the default value's type on read (`config/db_settings.py`: `_coerce_json_setting`), and the pattern collectors share one source parser (`modules/mail_parts/patterns.py`: `_as_pattern_mapping`, `_get_subject_patterns`).

### Added
- RU: Тесты на оба сценария: `tests/test_supplier_stock_mail.py` (синхронизация правил обработки с источником, очистка архива с датой письма) и `tests/test_backup_patterns_sources.py` (паттерны из строковой настройки, приоритет таблицы `backup_patterns`).
- EN: Tests for both scenarios: `tests/test_supplier_stock_mail.py` (processing rule sync with its source, archive cleanup with the email date) and `tests/test_backup_patterns_sources.py` (patterns from a string setting, `backup_patterns` table precedence).

## [8.63.40] - 2026-07-07

### Fixed
- RU: Письма «Config backup <host> OK» больше не распознаются как бэкапы БД. Пользовательские паттерны БД из таблицы `backup_patterns` могут быть без якорей (например `Backup (\w+) OK`), и такой паттерн матчил тему письма конфигов/историй: запись уходила в `database_backups` (лог «✅ Сохранен бэкап БД: pve13 - success»), таблица `config_console_backups` не пополнялась, и утренний отчёт показывал «🗂️ Бэкап конфигов и историй → Нет свежих отчётов: pve11, pve13, pve3», хотя письма приходили ежедневно. Теперь `parse_email_file` вызывает `parse_config_console_backup` (паттерн заанкорен `^Config backup …$` и чужие письма не перехватывает) ДО `parse_database_backup` (`modules/mail_parts/parsers/proxmox.py`).
- EN: "Config backup <host> OK" emails are no longer recognized as database backups. User-added DB patterns from the `backup_patterns` table can be unanchored (e.g. `Backup (\w+) OK`), and such a pattern matched the config/console backup subject: the record went into `database_backups` (log "✅ Сохранен бэкап БД: pve13 - success"), `config_console_backups` was never populated, and the morning report showed "🗂️ Config/console backup → No fresh reports: pve11, pve13, pve3" even though the emails arrived daily. `parse_email_file` now calls `parse_config_console_backup` (the pattern is anchored `^Config backup … $` and cannot steal other emails) BEFORE `parse_database_backup` (`modules/mail_parts/parsers/proxmox.py`).
- RU: Утренний отчёт ложно показывал «Нет бэкапов за 16ч — Клиенты: Buh2025» (и «Клиенты: 8/9»), хотя детали той же базы показывали успешные бэкапы каждый день. Парсер писем сохраняет имя БД в нижнем регистре (`buh2025`, из `subject_lower`), а сводки `get_database_backup_stats` и `get_backup_summary` сравнивали его с ключом конфига (`Buh2025`) точным регистрозависимым сравнением — та же причина, что у починенных в 8.63.30 деталей БД, но в сводках сравнение не нормализовали. Теперь обе сводки сравнивают имена через ту же нормализацию `_normalize_db_key` (регистр + `-`→`_`), имена в тексте отчёта остаются в написании из конфига (`extensions/backup_monitor/backup_utils.py`).
- EN: The morning report falsely showed "No backups in 16h — Clients: Buh2025" (and "Clients: 8/9") while the same database's details showed daily successful backups. The mail parser stores the database name in lower case (`buh2025`, from `subject_lower`), while the `get_database_backup_stats` and `get_backup_summary` summaries compared it with the config key (`Buh2025`) using exact case-sensitive comparison — the same root cause as the DB details fixed in 8.63.30, but the summaries were left unnormalized. Both summaries now compare names via the same `_normalize_db_key` normalization (case + `-`→`_`); names in the report text keep the config spelling (`extensions/backup_monitor/backup_utils.py`).

## [8.63.39] - 2026-07-03

### Fixed
- RU: Telegram-бот — починены остальные «мёртвые» мастера текстового ввода расширения backup_monitor: добавление серверов и паттернов «Бэкапа конфигов и историй» (`cc_add_server`, `cc_add_pattern`), паттерна «Передачи на NAS» (`nas_add_pattern`), добавление и переименование хостов Proxmox из backup-меню (`backup_add_proxmox_host`, `backup_edit_proxmox_host_name`). Причина та же, что у починенного ранее «Ожидаемого кол-ва файлов» (8.63.35): флаги проверялись только в `backup_host_settings_input_handler` (`extensions/backup_monitor/bot_handler.py`), до которого текстовый ввод никогда не доходит — его первым перехватывает `handle_setting_value` (`bot/handlers/settings_handlers/settings_value.py`, та же group=0 диспетчера python-telegram-bot). Теперь при любом из этих флагов ввод делегируется оригинальному обработчику целиком (`bot/handlers/settings_handlers/settings_value.py`).
- EN: Telegram bot — fixed the remaining "dead" text-input wizards of the backup_monitor extension: adding servers and patterns for "Config/console backup" (`cc_add_server`, `cc_add_pattern`), the "NAS transfer" pattern (`nas_add_pattern`), and adding/renaming Proxmox hosts from the backup menu (`backup_add_proxmox_host`, `backup_edit_proxmox_host_name`). Same root cause as the previously fixed "Expected file count" (8.63.35): the flags were only checked in `backup_host_settings_input_handler` (`extensions/backup_monitor/bot_handler.py`), which text input never reaches — `handle_setting_value` (`bot/handlers/settings_handlers/settings_value.py`, same python-telegram-bot dispatcher group=0) intercepts it first. Input is now delegated wholesale to the original handler when any of these flags is set (`bot/handlers/settings_handlers/settings_value.py`).
- RU: Android — утренний отчёт всё ещё мог приходить в «уплывшее» время (6:0x вместо 9:0x по расписанию): унаследованная от старых версий периодическая задача WorkManager продолжает срабатывать по сдвинутому расписанию, пока не выполнится хотя бы один запуск с новым кодом. Добавлено самоисцеление: (1) `MorningReportWorker.doWork()` при срабатывании более чем за 30 минут ДО целевого времени не запрашивает отчёт, а только переустанавливает корректное время следующего запуска (`setNextScheduleTimeOverride`) — первый же «ранний» запуск после обновления APK переносит отчёт на сегодняшние HH:MM; запуски ПОЗЖЕ целевого времени легитимны (Doze) и обрабатываются как раньше; (2) точное время переустанавливается при каждом старте приложения (`MonitoringApplication.onCreate`); (3) `rescheduleBackgroundWorkers` при пустом `metricsTimeInput` (настройки ещё не загружены с сервера) больше не затирает ранее синхронизированное время дефолтом «08:30», а берёт сохранённое в `AppPreferences.morningReportScheduleTime`. `ANDROID_VERSION_CODE` 891 → 892 — исправление требует установки обновлённого APK.
- EN: Android — the morning report could still arrive at a "drifted" time (6:0x instead of the scheduled 9:0x): the periodic WorkManager task inherited from older versions keeps firing on the shifted schedule until at least one run executes with the new code. Added self-healing: (1) `MorningReportWorker.doWork()`, when fired more than 30 minutes BEFORE the target time, does not request the report and only re-anchors the next run time (`setNextScheduleTimeOverride`) — the very first "early" run after the APK update moves the report to today's HH:MM; runs AFTER the target time are legitimate (Doze) and are handled as before; (2) the exact time is re-anchored on every app start (`MonitoringApplication.onCreate`); (3) `rescheduleBackgroundWorkers` with a blank `metricsTimeInput` (server settings not fetched yet) no longer overwrites the previously synced time with the "08:30" default — it uses the persisted `AppPreferences.morningReportScheduleTime` instead. `ANDROID_VERSION_CODE` 891 → 892 — the fix requires installing the updated APK.
...
### Fixed
- RU: Telegram — секции утреннего/ручного отчёта теперь действительно скрывают подробности в свёрнутом состоянии. Telegram сворачивает `<blockquote expandable>` только когда контент длиннее ~3 строк, поэтому короткие секции («🟢 Хостов: 15/15 (100%)» и т.п.) показывались целиком, хотя должны были быть спрятаны под заголовком. Детали секций теперь вообще не кладутся в текст сообщения: в тексте остаются только заголовки с флагами (и блок «⚠️ Требует внимания»), а под сообщением рисуются inline-кнопки секций («▸ 💾 Бэкапы Proxmox», по 2 в ряд, плюс «⬇️ Развернуть всё»/«⬆️ Свернуть всё»). Тап по кнопке редактирует сообщение, разворачивая/сворачивая тело секции (`<blockquote>` под её заголовком); состояние раскрытия кодируется битовой маской в `callback_data` (`mrs|<report_id>|<секция|a|n>|<маска>`), payload отчёта хранится в in-memory реестре (`modules/morning_report.py`: `register_report_payload`/`get_report_payload`/`build_report_keyboard`/`handle_report_section_callback`; лимит 8 отчётов). После перезапуска сервиса кнопки старых сообщений отвечают алертом «отчёт устарел». Задействовано и в плановой отправке (`send_report` → `lib/alerts.send_alert`, новый параметр `telegram_reply_markup`), и в ручном отчёте из меню (`core/monitor_parts/telegram_handlers.py`); маршрутизация callback'а добавлена в `bot/handlers/callbacks.py` до общего `query.answer()`, чтобы работал alert. Matrix (`<details>` сворачивается корректно вне зависимости от длины) и Android (нативные сворачиваемые карточки из 8.63.36) не менялись.
- EN: Telegram — morning/manual report sections now truly hide their details when collapsed. Telegram only collapses `<blockquote expandable>` when the content is longer than ~3 lines, so short sections ("🟢 Хостов: 15/15 (100%)" etc.) were shown in full even though they were supposed to be hidden under the header. Section details are no longer put into the message text at all: the text keeps only the titles with status flags (and the "⚠️ Требует внимания" block), while inline section buttons are rendered under the message ("▸ 💾 Бэкапы Proxmox", 2 per row, plus "⬇️ Развернуть всё"/"⬆️ Свернуть всё"). Tapping a button edits the message, expanding/collapsing the section body (`<blockquote>` under its header); the expansion state is encoded as a bitmask in `callback_data` (`mrs|<report_id>|<section|a|n>|<mask>`), and the report payload is kept in an in-memory registry (`modules/morning_report.py`: `register_report_payload`/`get_report_payload`/`build_report_keyboard`/`handle_report_section_callback`; capped at 8 reports). After a service restart, buttons of old messages answer with a "report is stale" alert. Wired into both the scheduled delivery (`send_report` → `lib/alerts.send_alert`, new `telegram_reply_markup` parameter) and the manual report from the menu (`core/monitor_parts/telegram_handlers.py`); callback routing added to `bot/handlers/callbacks.py` before the router's generic `query.answer()` so that alerts can be shown. Matrix (`<details>` collapses correctly regardless of length) and Android (native collapsible cards since 8.63.36) are unchanged.

## [8.63.36] - 2026-07-02

### Added
- RU: Android-приложение — экран «🌅 Последний отчёт» и диалог утреннего отчёта теперь рендерят сворачиваемые секции (заголовок с флагом состояния всегда виден, подробности скрыты до тапа — точно как в Telegram/Matrix). Бэкенд (`extensions/web_interface/__init__.py`, `/v1/control/actions`) отдаёт для `send_morning_report` структурированный JSON-payload (`morning_report`: report_type/app_version/generated_at/has_issues/problem_areas/composition/sections), построенный из того же `MorningReport.build_report()`, что уже управляет сворачиванием в Telegram/Matrix (`modules/morning_report.py`, метод `_report_to_json_payload`). В Android добавлены модели `MorningReportPayload`/`MorningReportSection` (`ApiModels.kt`), новый composable `MorningReportSectionsView`/`MorningReportSectionCard` (`MainActivity.kt`) с раскрывающимся chevron-заголовком (паттерн уже использовавшийся для секций SSH/Windows-доступа), персистентность структуры между запусками через `AppPreferences.morningReportSectionsJson` (JSON, сериализуется/десериализуется через новый `ApiFactory.morningReportToJson`/`-FromJson`) и обновлённый `MorningReportWorker` (фоновая push-доставка отчёта). При отсутствии структурированных данных (старый бэкенд, отчёт получен до обновления) экран откатывается на прежний плоский рендер.
- EN: Android app — the "🌅 Последний отчёт" screen and the morning-report dialog now render collapsible sections (the title with a status flag is always visible, details are hidden until tapped — matching the Telegram/Matrix behavior). The backend (`extensions/web_interface/__init__.py`, `/v1/control/actions`) now returns a structured JSON payload for `send_morning_report` (`morning_report`: report_type/app_version/generated_at/has_issues/problem_areas/composition/sections), built from the same `MorningReport.build_report()` that already drives collapsing in Telegram/Matrix (`modules/morning_report.py`, `_report_to_json_payload` method). Android gained `MorningReportPayload`/`MorningReportSection` models (`ApiModels.kt`), a new `MorningReportSectionsView`/`MorningReportSectionCard` composable (`MainActivity.kt`) with a chevron-toggle header (the pattern already used for the SSH/Windows access sections), persistence of the structure across app restarts via `AppPreferences.morningReportSectionsJson` (JSON, serialized/deserialized via the new `ApiFactory.morningReportToJson`/`-FromJson`), and an updated `MorningReportWorker` (background push delivery). When structured data is unavailable (older backend, report received before the update), the screen falls back to the previous flat-text rendering.

### Changed
- RU: Утренний/ручной отчёт — теперь сворачиваются ВСЕ секции без исключения (не только без проблем), оставляя только заголовок с флагом состояния; подробности скрыты до разворота (Telegram `<blockquote expandable>`, Matrix `<details>`). Список проблемных секций по-прежнему виден сразу в блоке «⚠️ Требует внимания» наверху сообщения. Секция «📸 Передачи снэпшотов» упрощена: убраны построчные детали по каждому хосту (статус/время), оставлена только общая сводка «Хостов: N/N (%)» — детализация по хосту доступна в отдельном меню «Передачи снэпшотов» (Telegram/Matrix) (`modules/morning_report.py`).
- EN: The morning/manual report now collapses ALL sections without exception (not just problem-free ones), leaving only the title with a status flag; details are hidden until expanded (Telegram `<blockquote expandable>`, Matrix `<details>`). The list of problem sections is still visible immediately in the "⚠️ Требует внимания" block at the top of the message. The "📸 Передачи снэпшотов" section was simplified: per-host detail lines (status/time) were removed, leaving only the aggregate "Хостов: N/N (%)" summary — per-host detail is available in the dedicated "Передачи снэпшотов" menu (Telegram/Matrix) (`modules/morning_report.py`).

### Fixed
- RU: Telegram-бот — «📦 Остатки 1С» → «🔢 Ожидаемое кол-во файлов» не сохранял введённое значение. Причина: флаг ожидания ввода (`stock_set_expected_files`) проверялся только в `extensions/backup_monitor/bot_handler.py` (`backup_host_settings_input_handler`), а весь текстовый ввод бота сначала перехватывает `handle_setting_value` (`bot/handlers/settings_handlers/settings_value.py`) — оба `MessageHandler` зарегистрированы в одной группе (`group=0`) диспетчера `python-telegram-bot`, и до второго обработчика дело никогда не доходило (диспетчер останавливает перебор хендлеров группы на первом совпавшем по фильтру). Флаг теперь проверяется прямо в `handle_setting_value`, по образцу уже работающего `nas_add_ignore_base`.
- EN: Telegram bot — "📦 Остатки 1С" → "🔢 Ожидаемое кол-во файлов" did not save the entered value. Cause: the pending-input flag (`stock_set_expected_files`) was only checked in `extensions/backup_monitor/bot_handler.py` (`backup_host_settings_input_handler`), while all bot text input is first intercepted by `handle_setting_value` (`bot/handlers/settings_handlers/settings_value.py`) — both `MessageHandler`s are registered in the same `python-telegram-bot` dispatcher group (`group=0`), so the second handler was never reached (the dispatcher stops iterating a group's handlers at the first filter match). The flag is now checked directly in `handle_setting_value`, following the already-working `nas_add_ignore_base` pattern.

## [8.63.34] - 2026-07-02

### Changed
- RU: Утренний/ручной отчёт полностью переработан (`modules/morning_report.py`). Секции без проблем теперь сворачиваются: в Telegram виден только заголовок с флагом состояния, детали раскрываются по тапу (`<blockquote expandable>`, отправка отчёта переведена на HTML), в Matrix — `<details>` в `formatted_body`; проблемные секции всегда раскрыты. Все секции приведены к единому формату «N/N (%)»: доступность серверов одной строкой («🟢 Серверы: 49/49 (100%)»), объединённая секция «Бэкапы» разбита на три отдельные по расширениям («💾 Бэкапы Proxmox», «🗃️ Бэкапы БД», «📬 Бэкапы почты»), «Бэкап конфигов и историй», «Передача на NAS», «Загрузка остатков 1С», «Статусы ZFS», «TLS-сертификаты», «Остатки поставщиков» и «Ресурсы серверов» переформатированы. «Свободное место ZFS» и «Передачи снэпшотов» вынесены из секции ZFS в самостоятельные секции: свободное место показывает процент по каждому пулу («🟢 host pool: свободно 62.5%»), передачи снэпшотов считаются по последнему статусу каждого хоста (как в статистике «📸 Передачи ZFS-снэпшотов»), а не по сырым записям писем (раньше STARTED-письмо ошибочно считалось ошибкой: «Передач: 2 (🟢 1 / 🔴 1)»). `lib/alerts.send_alert` получил параметры `telegram_html`/`matrix_html` (HTML-доставка с фолбэком на плоский текст), ручной отчёт из Telegram-меню отправляется в HTML. Плоский текст (Android, фолбэк) больше не содержит Markdown-разметки.
- EN: The morning/manual report was fully redesigned (`modules/morning_report.py`). Problem-free sections are now collapsed: Telegram shows only the title with a status flag and details expand on tap (`<blockquote expandable>`, report delivery switched to HTML), Matrix uses `<details>` in `formatted_body`; sections with issues are always expanded. All sections use a unified "N/N (%)" format: server availability as a single line ("🟢 Серверы: 49/49 (100%)"), the combined "Backups" section was split into three per-extension sections ("💾 Proxmox backups", "🗃️ DB backups", "📬 Mail backups"), and the config/console, NAS transfer, 1C stock load, ZFS status, TLS certificates, supplier stock and server resources sections were reformatted. "ZFS free space" and "Snapshot transfers" moved out of the ZFS section into standalone sections: free space shows the percent per pool ("🟢 host pool: свободно 62.5%") and snapshot transfers are evaluated by each host's latest status (as in the "📸 ZFS snapshot transfers" statistics view) instead of raw mail records (previously a STARTED mail wrongly counted as a failure: "Передач: 2 (🟢 1 / 🔴 1)"). `lib/alerts.send_alert` gained `telegram_html`/`matrix_html` parameters (HTML delivery with plain-text fallback), the manual report from the Telegram menu is sent as HTML. Plain text (Android, fallback) no longer contains Markdown markup.

### Added
- RU: Настройка ожидаемого количества файлов загрузки остатков 1С: ключ `STOCK_LOAD_EXPECTED_FILES` в БД настроек (0 — не задано) редактируется из Telegram-меню «📦 Остатки 1С» → «🔢 Ожидаемое кол-во файлов». Отчёт сверяет успешные загрузки с этим числом («🟢 Файлов: 44/44 (100%)») и подсвечивает нехватку файлов, а не только ошибки распарсенных писем (`extensions/backup_monitor/backup_utils.py`, `backup_handlers.py`, `bot_handler.py`, `core/config_manager.py`).
- EN: Setting for the expected number of 1C stock-load files: the `STOCK_LOAD_EXPECTED_FILES` settings-DB key (0 — not set) is editable from the Telegram menu "📦 Остатки 1С" → "🔢 Ожидаемое кол-во файлов". The report validates successful loads against this number ("🟢 Файлов: 44/44 (100%)") and highlights missing files, not just failures of parsed mails (`extensions/backup_monitor/backup_utils.py`, `backup_handlers.py`, `bot_handler.py`, `core/config_manager.py`).

### Fixed
- RU: Утренний отчёт не всегда приходил в Telegram/Matrix в назначенное время (например 9:01). Планировщик (`core/monitor.py`, `check_morning_report`) срабатывал только в узком окне `[план, план+CHECK_INTERVAL)`: если итерация основного цикла (опрос всех серверов с сетевыми таймаутами) длилась дольше интервала, окно проскакивалось и отчёт молча пропадал на весь день. Теперь слот срабатывает при любом «время наступило и слот ещё не отправлялся», т.е. отчёт при задержке приходит позже, но приходит всегда; отправленные слоты сохраняются в БД настроек (`MORNING_REPORT_SENT_SLOTS`), чтобы перезапуск сервиса после планового времени не дублировал отчёт.
- EN: The morning report did not always arrive in Telegram/Matrix at the scheduled time (e.g. 9:01). The scheduler (`core/monitor.py`, `check_morning_report`) only fired within a narrow `[plan, plan+CHECK_INTERVAL)` window: when a main-loop iteration (polling all servers with network timeouts) took longer than the interval, the window was skipped and the report silently disappeared for the whole day. Now a slot fires on "time reached and slot not sent yet", so a delayed report arrives late but always arrives; sent slots are persisted in the settings DB (`MORNING_REPORT_SENT_SLOTS`) so a service restart after the scheduled time does not duplicate the report.
- RU: Android: утренний отчёт приходил в приложение в «плывущее» время (последний — в 6:07 вместо 9:01). `MorningReportWorker` использовал `PeriodicWorkRequest` с `setInitialDelay`: точным было только первое срабатывание, дальше каждое отложенное системой выполнение (Doze, перезагрузка) навсегда сдвигало расписание. Теперь точное время запуска задаётся через `setNextScheduleTimeOverride` (WorkManager 2.9) и переустанавливается после каждого выполнения `doWork()`; время расписания сохраняется в `AppPreferences.morningReportScheduleTime` (`android-client/.../notifications/MorningReportWorker.kt`, `storage/AppPreferences.kt`).
- EN: Android: the morning report arrived at a "drifting" time (last one at 6:07 instead of 9:01). `MorningReportWorker` used a `PeriodicWorkRequest` with `setInitialDelay`: only the first run was exact, after that every OS-deferred execution (Doze, reboot) permanently shifted the schedule. The exact run time is now set via `setNextScheduleTimeOverride` (WorkManager 2.9) and re-anchored after every `doWork()` execution; the schedule time is persisted in `AppPreferences.morningReportScheduleTime` (`android-client/.../notifications/MorningReportWorker.kt`, `storage/AppPreferences.kt`).
...
### Added
- RU: Android/сервер — нативное редактирование базовых полей правил почты, ресурсов выгрузки и правил обработки остатков поставщиков (раньше редактирование было доступно только в Telegram-боте). В соответствующих списках у каждой записи появилась кнопка «✏️ Изменить»: открывается экран с текущими значениями и полями для точечного изменения — пустое поле оставляет значение без изменений. Правило почты: название, отправитель/тема/имя вложения (regex), число вложений, шаблон выходного файла. Ресурс: название, UNC-путь, логин, пароль (скрытый ввод). Правило обработки: название, файл источника. Сервер (BFF `/v1/settings/extensions/actions`) реализует действия `supplier_stock_mail_source_edit|<id>` / `…_update|<id>|…`, `supplier_stock_resource_edit|<id>` / `…_update|<id>|…`, `supplier_stock_proc_edit|<id>` / `…_update|<id>|…` с обновлением только заполненных полей и сохранением в `mail.sources` / `resources` / `processing.rules`. MIME-фильтр почты, колонки/шаблоны/варианты обработки — пока в Telegram-боте.
- EN: Android/server — native editing of the basic fields of mail rules, upload resources and processing rules for supplier stock (editing was previously only available in the Telegram bot). In the respective lists each entry now has an "✏️ Edit" button: it opens a screen with current values and fields for targeted changes — an empty field leaves the value unchanged. Mail rule: name, sender/subject/attachment-name (regex), attachment count, output file template. Resource: name, UNC path, login, password (masked input). Processing rule: name, source file. The server (BFF `/v1/settings/extensions/actions`) implements the `supplier_stock_mail_source_edit|<id>` / `…_update|<id>|…`, `supplier_stock_resource_edit|<id>` / `…_update|<id>|…`, `supplier_stock_proc_edit|<id>` / `…_update|<id>|…` actions, updating only the filled fields and persisting to `mail.sources` / `resources` / `processing.rules`. The mail MIME filter and processing columns/templates/variants are still in the Telegram bot.
...
### Fixed
- RU: Android — плашка «📦 поставщики» показывала некорректное соотношение (например «5/17»). Универсальный парсер сводки неправильно разбирал строку-счётчик дашборда остатков поставщиков (`✅ N   🟡 N   🔴 N   всего N`), беря первое попавшееся число как «всего». Добавлен отдельный разбор этой строки: числитель — поставщики без проблем (✅), знаменатель — всего поставщиков; индикатор проблемы загорается при наличии 🟡/🔴.
- EN: Android — the "📦 suppliers" tile showed a wrong ratio (e.g. "5/17"). The generic summary parser misread the supplier-stock dashboard counter line (`✅ N   🟡 N   🔴 N   total N`), taking the first number as the total. Added a dedicated parser for that line: the numerator is the suppliers without problems (✅), the denominator is the total number of suppliers; the problem indicator lights up when 🟡/🔴 are present.
- RU: Android/сервер — клик по кнопке с именем поставщика в диалоге «📦 Остатки поставщиков» приводил к сообщению «Пока нет данных об остатках поставщиков». Причина — BFF-эндпоинт `/v1/control/actions` не маршрутизировал действия `supplier_stock_reports_download/mail` и `supplier_stock_report_source_day|…` в обработчик (они не входили в список `menu_actions` и не покрывались префиксами), поэтому возвращалась ошибка. Действия добавлены в условие входа обработчика и в проверку включённости расширения.
- EN: Android/server — tapping a supplier-name button in the "📦 Supplier stock" dialog resulted in "No supplier stock data yet". Root cause: the `/v1/control/actions` BFF endpoint did not route the `supplier_stock_reports_download/mail` and `supplier_stock_report_source_day|…` actions into their handler (they were neither in `menu_actions` nor covered by a prefix), so it returned an error. The actions were added to the handler's entry condition and to the extension-enabled guard.

### Added
- RU: Android — обновление приложения прямо из приложения с выбором ветки. В разделе «⚙️ Настройки» появилась карточка «⬆️ Обновление приложения»: выбор ветки обновления (Develop/Main и др.), кнопка «🔎 Проверить обновления» (сравнивает установленную версию с актуальной версией ветки) и кнопка «⬇️ Скачать и установить» — APK качается через DownloadManager и открывается системный установщик (нужно разовое разрешение «установка из неизвестных источников»). Если ссылка ведёт на страницу релизов, а не на .apk, она открывается в браузере. Сервер отдаёт список веток через `GET /v1/mobile/branches`, конкретную ветку — через `GET /v1/mobile/version?branch=<name>`; ветки настраиваются в `config/settings.py` (`ANDROID_UPDATE_BRANCHES`, `ANDROID_DEFAULT_UPDATE_BRANCH`, `ANDROID_RELEASE_REPO`).
- EN: Android — in-app updates with branch selection. The "⚙️ Settings" screen now has an "⬆️ App update" card: pick an update branch (Develop/Main, etc.), a "🔎 Check for updates" button (compares the installed version with the branch's latest version) and a "⬇️ Download and install" button — the APK is fetched via DownloadManager and the system installer is opened (a one-time "install from unknown sources" permission is required). If the link points to a releases page rather than an .apk, it opens in the browser. The server returns the branch list via `GET /v1/mobile/branches` and a specific branch via `GET /v1/mobile/version?branch=<name>`; branches are configured in `config/settings.py` (`ANDROID_UPDATE_BRANCHES`, `ANDROID_DEFAULT_UPDATE_BRANCH`, `ANDROID_RELEASE_REPO`).
...
### Changed
- RU: Веб-интерфейс полностью переработан — вместо статичного дашборда теперь современный одностраничный SPA с тёмной/светлой темой, боковой навигацией, адаптивной вёрсткой и достижением паритета функций с Android-приложением. Доступны: вход по логину/паролю (тот же BFF-токен, что и у мобильного клиента), дашборд доступности с карточками и индикаторами CPU/RAM/диск по каждому серверу, управление мониторингом (пауза/возобновление, режим тишины авто/громкий/тихий, утренний отчёт), полное управление серверами (добавление/редактирование/включение/удаление), все настройки (мониторинг, Telegram, Matrix, время, SSH/Windows-доступы с учётными записями и типами, состав отчёта, расширения с переключателями) и раздел «Операции» (бэкапы Proxmox/БД/почты/1С/NAS/конфигов, ZFS, свободное место ZFS, снэпшоты, TLS-сертификаты, остатки поставщиков). Веб-интерфейс работает поверх существующего `v1` BFF API, который использует Android-клиент, — контракт API не менялся, поэтому мобильное приложение продолжает работать без изменений.
- EN: The web interface was fully reworked — instead of a static dashboard it is now a modern single-page app with dark/light themes, sidebar navigation, responsive layout and feature parity with the Android app. It offers: login by username/password (the same BFF token the mobile client uses), an availability dashboard with cards and per-server CPU/RAM/disk indicators, monitoring control (pause/resume, silent mode auto/loud/quiet, morning report), full server management (add/edit/enable/delete), all settings (monitoring, Telegram, Matrix, time, SSH/Windows credentials with accounts and types, report composition, extensions with toggles) and an "Operations" section (Proxmox/DB/mail/1C/NAS/config backups, ZFS, ZFS free space, snapshots, TLS certificates, supplier stock). The web interface runs on top of the existing `v1` BFF API used by the Android client — the API contract was not changed, so the mobile app keeps working unchanged.
...
### Added
- RU: Новое расширение `config_console_backup_monitor` — мониторинг копирования конфигов VM/LXC и историй консолей хостов на бэкап-сервер/NAS. Новый единый скрипт `scripts/copy_configs_ssh.sh` (байт-идентичен на всех хостах и бэкап-серверах; различия — в опциональном `/etc/copy_configs.conf`, см. `scripts/copy_configs.conf.example`): доставка переведена с сетевой шары на **SSH (rsync)** с настраиваемым приёмником (по умолчанию бэкап-сервер); ручной список `CONTAINER_IDS` убран — LXC/VM определяются автоматически через `pct list`/`qm list`; нетиповые subvol-пути контейнеров (`subvol-141-disk-0`, `subvol-142-disk-1`, …) больше не хардкодятся (запущенные читаются через `pct exec`, остановленные — через `pct config` + `zfs mountpoint` с фолбэком на glob); собираются `.bash_history` всех пользователей контейнеров (root, postgres, …) и опционально `.psql_history`. Итоговое письмо «Config backup <host> OK|PARTIAL|ERROR» уходит на `katok@202020.ru` и парсится новым mixin `ConfigConsoleBackupParserMixin` в таблицу `config_console_backups`. Этот PR: скрипты + backend (парсер/таблица/паттерн/регистрация расширения + getter `get_config_console_backups`); Telegram/Matrix/Android — следующим PR.
- EN: New `config_console_backup_monitor` extension — monitors copying of VM/LXC configs and host console histories to the backup server/NAS. New unified `scripts/copy_configs_ssh.sh` (byte-identical on every host and backup server; per-machine differences live in optional `/etc/copy_configs.conf`, see `scripts/copy_configs.conf.example`): delivery switched from a network share to **SSH (rsync)** with a configurable receiver (backup server by default); the manual `CONTAINER_IDS` list is gone — LXC/VM are auto-discovered via `pct list`/`qm list`; non-standard container subvol paths (`subvol-141-disk-0`, `subvol-142-disk-1`, …) are no longer hardcoded (running containers read via `pct exec`, stopped ones via `pct config` + `zfs mountpoint` with a glob fallback); collects `.bash_history` for every container user (root, postgres, …) and optionally `.psql_history`. The summary email "Config backup <host> OK|PARTIAL|ERROR" is sent to `katok@202020.ru` and parsed by the new `ConfigConsoleBackupParserMixin` mixin into the `config_console_backups` table. This PR: scripts + backend (parser/table/pattern/extension registration + `get_config_console_backups` getter); Telegram/Matrix/Android to follow.
...
- EN: Fixed a regression from the PR11 `settings_handlers` split: `settings_callback_handler` moved into `callback_dispatcher.py` but referenced names left in `_legacy.py` (`settings_command`, `_safe_query_answer`, `show_telegram_settings`, etc.) that were not bound in the new module's namespace. Any settings callback crashed with `NameError: name 'settings_command' is not defined`. A static import of `_legacy` into `callback_dispatcher` is impossible (circular — `_legacy` itself does `from .callback_dispatcher import *`), so the names are bound lazily on first call via `_ensure_legacy_bindings()` (`setdefault`, without clobbering the module's own names). Added regression test `test_settings_dispatcher_resolves_legacy_names`.

### Added
- RU: Новое расширение `nas_transfer_monitor` («📤 Передача бэкапов на NAS») — мониторинг результатов скрипта `move_and_clear_backups.sh` на сервере бэкапов, который копирует бэкапы 1С на NAS. Скрипт шлёт одно итоговое письмо на прогон (тема `NAS transfer <host> OK|ERROR|SKIPPED`, тело с полями «Начало/Завершено/NAS примонтирован/Баз обработано/Ошибок/Проблемные базы»), которое пересылается на почтовый сервер проекта и парсится. Новый mixin `NasTransferParserMixin` (`modules/mail_parts/parsers/nas_transfer.py`) подключён в `BackupProcessor` и диспетчере `parse_email_file`; данные хранятся в новой таблице `nas_transfers` (одна запись на прогон, дедуп через `INSERT OR IGNORE`). Отображение добавлено в Telegram-бот (кнопка «📤 Передача на NAS» в меню бэкапов), Matrix-бот (`!nas`, пункт `!extensions`) и Android-приложение (плитка + control action `backup_nas_transfer`). Настройка расширения (вкл/выкл и `NAS_TRANSFER_ALERT_HOURS`) доступна во всех трёх клиентах.
- EN: New extension `nas_transfer_monitor` ("📤 NAS backup transfer") — monitors the results of the `move_and_clear_backups.sh` script on the backup server that copies 1C backups to a NAS. The script sends one summary email per run (subject `NAS transfer <host> OK|ERROR|SKIPPED`, body with "started/finished/NAS mounted/bases processed/errors/problem bases" fields) that is forwarded to the project mail server and parsed. A new `NasTransferParserMixin` (`modules/mail_parts/parsers/nas_transfer.py`) is wired into `BackupProcessor` and the `parse_email_file` dispatcher; data is stored in the new `nas_transfers` table (one row per run, dedup via `INSERT OR IGNORE`). Display added to the Telegram bot (a "📤 NAS transfer" button in the backups menu), Matrix bot (`!nas`, `!extensions` entry) and Android app (tile + `backup_nas_transfer` control action). Extension settings (on/off and `NAS_TRANSFER_ALERT_HOURS`) are available in all three clients.
...
### Changed
- RU: PR7e серии оптимизации — из `bot/handlers/settings_handlers/_legacy.py` вынесена самая крупная оставшаяся семья: **53 функции** настроек бэкапов баз 1C/Cobian в новый модуль `bot/handlers/settings_handlers/backups/db.py` (~1812 строк). Сюда вошли: главные UI-меню (`show_database_backup_settings`, `show_backup_databases_settings`, `show_backup_databases`, `show_db_patterns_menu`, `show_db_patterns_menu_from_backup`); CRUD над категориями БД (`add_database_category_handler`, `edit_database_category_handler`, `delete_database_category_handler`, `manage_database_categories_handler`, `edit_databases_handler`, `start_database_category_edit_handler`, `delete_database_category_confirmation`, `delete_database_category_execute`, `edit_database_category_details`); CRUD над записями внутри категорий (`add_database_entry_handler`, `edit_database_entry_handler`, `delete_database_entry_confirmation`, `delete_database_entry_execute`, `add_new_database_from_manage_handler`, `view_all_databases_handler`); toggle мониторинга (`settings_toggle_database_monitoring`); default-паттерны (`edit_default_db_pattern_handler`, `delete_default_db_pattern_handler`, `handle_default_db_pattern_edit_input`, `_show_db_pattern_confirm`); input-handlers (`handle_db_category_input`, `handle_db_category_rename_input`, `handle_db_entry_input`, `handle_db_entry_edit_input`); токенизация callback-data (`_build_db_monitor_toggle_callback`, `_build_db_category_callback`, `_resolve_db_category_from_callback`, `_build_db_entry_callback`, `_resolve_db_entry_from_callback`, `_get_settings_db_back_callback`); helpers конфига (`_get_disabled_db_monitors_settings`, `_toggle_database_monitoring_settings`, `_get_database_fallback_patterns`, `_get_database_patterns_setting`, `_save_database_patterns_setting`, `_get_database_names`, `_inject_db_placeholder`, `_build_db_pattern_from_subject`, `_build_db_pattern_from_fragments`, `_get_database_categories`, `_get_database_category`). `_legacy.py` сжался с 7314 до **5593 строк** (−24%).
- EN: PR7e of the optimization series — the largest remaining family is moved out of `bot/handlers/settings_handlers/_legacy.py`: **53 functions** for 1C/Cobian database backup settings into the new module `bot/handlers/settings_handlers/backups/db.py` (~1812 lines). This includes: top-level UI menus (`show_database_backup_settings`, `show_backup_databases_settings`, `show_backup_databases`, `show_db_patterns_menu`, `show_db_patterns_menu_from_backup`); CRUD over DB categories (`add_database_category_handler`, `edit_database_category_handler`, `delete_database_category_handler`, `manage_database_categories_handler`, `edit_databases_handler`, `start_database_category_edit_handler`, `delete_database_category_confirmation`, `delete_database_category_execute`, `edit_database_category_details`); CRUD over entries inside categories (`add_database_entry_handler`, `edit_database_entry_handler`, `delete_database_entry_confirmation`, `delete_database_entry_execute`, `add_new_database_from_manage_handler`, `view_all_databases_handler`); monitoring toggle (`settings_toggle_database_monitoring`); default patterns (`edit_default_db_pattern_handler`, `delete_default_db_pattern_handler`, `handle_default_db_pattern_edit_input`, `_show_db_pattern_confirm`); input handlers (`handle_db_category_input`, `handle_db_category_rename_input`, `handle_db_entry_input`, `handle_db_entry_edit_input`); callback-data tokenization (`_build_db_monitor_toggle_callback`, `_build_db_category_callback`, `_resolve_db_category_from_callback`, `_build_db_entry_callback`, `_resolve_db_entry_from_callback`, `_get_settings_db_back_callback`); config helpers (`_get_disabled_db_monitors_settings`, `_toggle_database_monitoring_settings`, `_get_database_fallback_patterns`, `_get_database_patterns_setting`, `_save_database_patterns_setting`, `_get_database_names`, `_inject_db_placeholder`, `_build_db_pattern_from_subject`, `_build_db_pattern_from_fragments`, `_get_database_categories`, `_get_database_category`). `_legacy.py` shrunk from 7314 to **5593 lines** (−24%).
- RU: Двусторонний re-export расширен — `_legacy.py` теперь подсасывает `from .backups.db import *` рядом с уже существующими `supplier_stock`, `zfs`, `backups.proxmox`, `backups.snapshot`. Identity-check на трёх путях добавлен в `test_settings_handlers_backups_db_split` — пин-ит, что `show_database_backup_settings` и `settings_toggle_database_monitoring` идентичны через фасад, `_legacy.py` и через прямой импорт `backups.db`; `__module__` указывает в `backups.db`.
- EN: The two-way re-export is extended — `_legacy.py` now pulls in `from .backups.db import *` alongside the existing `supplier_stock`, `zfs`, `backups.proxmox`, `backups.snapshot`. The identity check on three paths is added in `test_settings_handlers_backups_db_split` — pinning that `show_database_backup_settings` and `settings_toggle_database_monitoring` are identical through the facade, `_legacy.py`, and a direct `backups.db` import; `__module__` points at `backups.db`.
- RU: Извлечение AST-скриптом `/tmp/extract_db.py` — regex `(database|_db_|db_pattern|default_db|db_monitor|db_category|db_entry|db_patterns)` отлавливает все 53 функции семьи. Внешние импортёры **не правлены** — `from bot.handlers.settings_handlers import show_database_backup_settings` продолжает работать через фасад. После PR7e в pytest **53 passed**.
- EN: Extracted via an AST script (`/tmp/extract_db.py`) — the regex `(database|_db_|db_pattern|default_db|db_monitor|db_category|db_entry|db_patterns)` captures all 53 functions of the family. External importers are **not touched** — `from bot.handlers.settings_handlers import show_database_backup_settings` keeps working via the facade. After PR7e pytest has **53 passed**.
- RU: Прогресс декомпозиции `settings_handlers`: 14 776 → 5593 строки в `_legacy.py` (38% исходного). Распределение по модулям: `supplier_stock.py` 5805 / `backups/db.py` 1812 / `zfs.py` 960 / `backups/proxmox.py` 594 / `backups/snapshot.py` 455 / `_legacy.py` 5593.
- EN: `settings_handlers` decomposition progress: 14 776 → 5593 lines in `_legacy.py` (38% of original). Module breakdown: `supplier_stock.py` 5805 / `backups/db.py` 1812 / `zfs.py` 960 / `backups/proxmox.py` 594 / `backups/snapshot.py` 455 / `_legacy.py` 5593.
- RU: SemVer patch-бамп до `8.62.59`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.59`, `ANDROID_VERSION_CODE=821`).
- EN: SemVer patch bump to `8.62.59`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.59`, `ANDROID_VERSION_CODE=821`).

## [8.62.58] - 2026-05-26

### Changed
- RU: PR7d серии оптимизации — из `bot/handlers/settings_handlers/_legacy.py` вынесены ещё две backup-семьи в новый подпакет `bot/handlers/settings_handlers/backups/`:
  - `backups/proxmox.py` (~594 строки, **15 функций**): UI Proxmox vzdump-настроек — `show_proxmox_backup_settings`, `show_proxmox_patterns_menu`, `show_backup_proxmox_settings`, `show_proxmox_hosts_list`; CRUD над списком Proxmox-хостов (`add_proxmox_host_handler`, `delete_proxmox_host`, `edit_proxmox_host_handler`, `handle_proxmox_host_input`, `handle_proxmox_host_edit_input`, `toggle_proxmox_host`); helpers `_normalize_proxmox_hosts`, `_get_proxmox_hosts_for_settings`; pattern-handlers `add_proxmox_pattern_handler`, `proxmox_pattern_retry_handler`, `proxmox_pattern_confirm_handler`.
  - `backups/snapshot.py` (~455 строк, **10 функций**): UI ZFS snapshot-transfer настроек — `show_snapshot_hosts_menu`, `show_snapshot_transfer_settings`; CRUD/toggle/delete hosts (`toggle_snapshot_host_handler`, `delete_snapshot_host_handler`, `handle_snapshot_host_text_input`); helpers `_get_snapshot_hosts_config`, `_save_snapshot_hosts_config`, `_clear_snapshot_host_input_state`; pattern-handlers `add_snapshot_pattern_handler`, `snapshot_pattern_confirm_handler`.
  Создан подпакет `backups/__init__.py` — точка входа будущих модулей `backups/db.py` и `backups/mail.py` (в PR7e+). `_legacy.py` сжался с 8232 до **7314 строк** (−11%).
- EN: PR7d of the optimization series — two more backup families are moved out of `bot/handlers/settings_handlers/_legacy.py` into the new sub-package `bot/handlers/settings_handlers/backups/`:
  - `backups/proxmox.py` (~594 lines, **15 functions**): Proxmox vzdump settings UI — `show_proxmox_backup_settings`, `show_proxmox_patterns_menu`, `show_backup_proxmox_settings`, `show_proxmox_hosts_list`; CRUD on the Proxmox hosts list (`add_proxmox_host_handler`, `delete_proxmox_host`, `edit_proxmox_host_handler`, `handle_proxmox_host_input`, `handle_proxmox_host_edit_input`, `toggle_proxmox_host`); helpers `_normalize_proxmox_hosts`, `_get_proxmox_hosts_for_settings`; pattern handlers `add_proxmox_pattern_handler`, `proxmox_pattern_retry_handler`, `proxmox_pattern_confirm_handler`.
  - `backups/snapshot.py` (~455 lines, **10 functions**): ZFS snapshot transfer settings UI — `show_snapshot_hosts_menu`, `show_snapshot_transfer_settings`; CRUD/toggle/delete hosts (`toggle_snapshot_host_handler`, `delete_snapshot_host_handler`, `handle_snapshot_host_text_input`); helpers `_get_snapshot_hosts_config`, `_save_snapshot_hosts_config`, `_clear_snapshot_host_input_state`; pattern handlers `add_snapshot_pattern_handler`, `snapshot_pattern_confirm_handler`.
  The new sub-package `backups/__init__.py` is the entry point for the future `backups/db.py` and `backups/mail.py` modules (in PR7e+). `_legacy.py` shrank from 8232 to **7314 lines** (−11%).
- RU: Двусторонний re-export расширен — `_legacy.py` теперь подсасывает `from .backups.proxmox import *` и `from .backups.snapshot import *` рядом с уже существующими импортами для `supplier_stock` и `zfs`. Это сохраняет работу внутренних ссылок `settings_callback_handler` на имена вроде `show_proxmox_backup_settings` без префикса.
- EN: The two-way re-export is extended — `_legacy.py` now pulls in `from .backups.proxmox import *` and `from .backups.snapshot import *` alongside the existing imports for `supplier_stock` and `zfs`. This keeps internal `settings_callback_handler` references like `show_proxmox_backup_settings` resolving without a prefix.
- RU: Извлечение AST-скриптом `/tmp/extract_backups.py` — общая «карта» extract'ов (EXTRACTS list) с regex по имени и путём назначения, чтобы добавление новых backup-семей в PR7e не требовало нового скрипта. Identity-check на трёх путях добавлен в `test_settings_handlers_backups_split` — пин-ит, что `show_proxmox_backup_settings` и `show_snapshot_transfer_settings` идентичны через фасад, `_legacy.py` и через прямой импорт нового модуля; `__module__` указывает в `backups.proxmox` / `backups.snapshot`.
- EN: Extracted via an AST script (`/tmp/extract_backups.py`) — a general "extract map" (EXTRACTS list) keyed by name regex and destination path, so adding new backup families in PR7e won't need a new script. An identity check on three paths is added in `test_settings_handlers_backups_split` — pinning that `show_proxmox_backup_settings` and `show_snapshot_transfer_settings` are identical through the facade, `_legacy.py`, and a direct new-module import; `__module__` points at `backups.proxmox` / `backups.snapshot`.
- RU: Внешние импортёры (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) **не правлены** — `bot/handlers/callbacks.py:368` (`from bot.handlers.settings_handlers import show_snapshot_transfer_settings`) продолжает работать через фасад. После PR7d в pytest **52 passed**.
- EN: External importers (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) are **not touched** — `bot/handlers/callbacks.py:368` (`from bot.handlers.settings_handlers import show_snapshot_transfer_settings`) keeps working via the facade. After PR7d pytest has **52 passed**.
- RU: SemVer patch-бамп до `8.62.58`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.58`, `ANDROID_VERSION_CODE=820`).
- EN: SemVer patch bump to `8.62.58`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.58`, `ANDROID_VERSION_CODE=820`).

## [8.62.57] - 2026-05-26

### Changed
- RU: PR7c серии оптимизации — из `bot/handlers/settings_handlers/_legacy.py` вынесена следующая самодостаточная семья: **29 функций ZFS-настроек** в новый модуль `bot/handlers/settings_handlers/zfs.py` (~933 строки). Сюда вошли все UI-меню ZFS-мониторинга (`show_zfs_main_menu`, `show_zfs_settings`, `show_zfs_status_summary`, `show_zfs_servers_list`, `show_zfs_patterns_menu`), CRUD-операции над списком ZFS-серверов (`add_zfs_server_handler`, `delete_zfs_server`, `edit_zfs_server_*_handler`, `toggle_zfs_server`, `handle_zfs_server_*_input`), DB-helpers состояния мониторинга ZFS-пулов (`_get_zfs_server_names`, `_ensure_zfs_monitoring_state_table`, `_get_zfs_monitoring_state_map`, `_set_zfs_monitoring_state`, `_rename_zfs_monitoring_state`, `_delete_zfs_monitoring_state`, `_delete_zfs_server_statuses`, `_rename_zfs_server_statuses`, `_build_zfs_current_status_lines`, `_build_zfs_pattern_from_subject`, `_build_zfs_pattern_from_fragments`) и pattern-handlers (`add_zfs_pattern_handler`, `zfs_pattern_retry_handler`, `zfs_pattern_confirm_handler`). `_legacy.py` сжался с 9221 до **8232 строк** (−11%).
- EN: PR7c of the optimization series — the next self-contained family is moved out of `bot/handlers/settings_handlers/_legacy.py`: **29 ZFS settings functions** into the new module `bot/handlers/settings_handlers/zfs.py` (~933 lines). This includes all ZFS monitoring UI menus (`show_zfs_main_menu`, `show_zfs_settings`, `show_zfs_status_summary`, `show_zfs_servers_list`, `show_zfs_patterns_menu`), CRUD on the ZFS server list (`add_zfs_server_handler`, `delete_zfs_server`, `edit_zfs_server_*_handler`, `toggle_zfs_server`, `handle_zfs_server_*_input`), DB helpers for ZFS pool monitoring state (`_get_zfs_server_names`, `_ensure_zfs_monitoring_state_table`, `_get_zfs_monitoring_state_map`, `_set_zfs_monitoring_state`, `_rename_zfs_monitoring_state`, `_delete_zfs_monitoring_state`, `_delete_zfs_server_statuses`, `_rename_zfs_server_statuses`, `_build_zfs_current_status_lines`, `_build_zfs_pattern_from_subject`, `_build_zfs_pattern_from_fragments`) and pattern handlers (`add_zfs_pattern_handler`, `zfs_pattern_retry_handler`, `zfs_pattern_confirm_handler`). `_legacy.py` shrank from 9221 to **8232 lines** (−11%).
- RU: Реэкспорт двусторонний — `_legacy.py` подсасывает `from .zfs import *`, чтобы внутренние ссылки `settings_callback_handler` на `show_zfs_main_menu` без префикса продолжали резолвиться. Identity-check на трёх путях: `from bot.handlers.settings_handlers import show_zfs_main_menu`, `from bot.handlers.settings_handlers._legacy import show_zfs_main_menu`, `from bot.handlers.settings_handlers.zfs import show_zfs_main_menu` — все три возвращают тот же объект функции, `__module__` указывает в `zfs.py`.
- EN: Re-export is two-way — `_legacy.py` pulls in `from .zfs import *` so internal `settings_callback_handler` references to `show_zfs_main_menu` without a prefix keep resolving. Identity check on three paths (`from bot.handlers.settings_handlers import show_zfs_main_menu`, `from bot.handlers.settings_handlers._legacy import show_zfs_main_menu`, `from bot.handlers.settings_handlers.zfs import show_zfs_main_menu`) returns the same function object, `__module__` points to `zfs.py`.
- RU: Извлечение AST-скриптом `/tmp/extract_zfs.py` — границы функций по дереву Python, выборка regex `zfs` в имени, 29 функций в порядке появления. Внешние импортёры (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) **не правлены** — `bot/handlers/callbacks.py:356` (`from bot.handlers.settings_handlers import show_zfs_main_menu`) продолжает работать через фасад.
- EN: Extracted by an AST script (`/tmp/extract_zfs.py`) — function boundaries from the Python tree, selection by `zfs` regex in the name, 29 functions in source order. External importers (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) are **not touched** — `bot/handlers/callbacks.py:356` (`from bot.handlers.settings_handlers import show_zfs_main_menu`) keeps working via the facade.
- RU: Smoke-тест расширен: `test_settings_handlers_zfs_split` пин-ит, что три пути импорта возвращают идентичный объект и `__module__` указывает в новый модуль. После PR7c в pytest **51 passed**.
- EN: Smoke test extended: `test_settings_handlers_zfs_split` pins that all three import paths return the identical object and `__module__` points to the new module. After PR7c pytest has **51 passed**.
- RU: SemVer patch-бамп до `8.62.57`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.57`, `ANDROID_VERSION_CODE=819`).
- EN: SemVer patch bump to `8.62.57`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.57`, `ANDROID_VERSION_CODE=819`).

## [8.62.56] - 2026-05-26

### Changed
- RU: PR7b серии оптимизации — из `bot/handlers/settings_handlers/_legacy.py` вынесен самый крупный самодостаточный блок: **84 функции** supplier-stock в новый `bot/handlers/settings_handlers/supplier_stock.py` (~5723 строк). Сюда вошли все UI-меню (`show_supplier_stock_settings`, `show_supplier_stock_reports`, `show_supplier_stock_sources_menu`, `show_supplier_stock_mail_settings`, `show_supplier_stock_processing_menu`, `show_supplier_stock_schedule_menu` и т.д.), мастера создания/редактирования источников и правил обработки (`supplier_stock_start_source_wizard`, `supplier_stock_start_edit_wizard`, `supplier_stock_start_processing_wizard` и др.), все handler-ы ввода (`supplier_stock_handle_input`, `supplier_stock_handle_*_field_input`), helper-функции (`_supplier_stock_*`, `_build_stock_subject_pattern`, `_format_supplier_stock_timestamp` и т.д.) и весь stock-load шаблонный pipeline (`show_stock_load_patterns_menu`, `stock_pattern_*_handler`). `_legacy.py` сжался с 14 776 до **9221 строки** (−37%).
- EN: PR7b of the optimization series — the largest self-contained block has been moved out of `bot/handlers/settings_handlers/_legacy.py`: **84 supplier-stock functions** into the new `bot/handlers/settings_handlers/supplier_stock.py` (~5723 lines). This includes all UI menus (`show_supplier_stock_settings`, `show_supplier_stock_reports`, `show_supplier_stock_sources_menu`, `show_supplier_stock_mail_settings`, `show_supplier_stock_processing_menu`, `show_supplier_stock_schedule_menu`, etc.), wizards for creating/editing sources and processing rules (`supplier_stock_start_source_wizard`, `supplier_stock_start_edit_wizard`, `supplier_stock_start_processing_wizard`, etc.), all input handlers (`supplier_stock_handle_input`, `supplier_stock_handle_*_field_input`), helper functions (`_supplier_stock_*`, `_build_stock_subject_pattern`, `_format_supplier_stock_timestamp`, etc.), and the entire stock-load pattern pipeline (`show_stock_load_patterns_menu`, `stock_pattern_*_handler`). `_legacy.py` shrunk from 14 776 to **9221 lines** (−37%).
- RU: Реэкспорт двусторонний: `bot/handlers/settings_handlers/__init__.py` через `from _legacy import *` собирает все имена (включая re-export из `supplier_stock`), а `_legacy.py` через `from supplier_stock import *` подсасывает обратно всё, что нужно его внутренним ссылкам (`settings_callback_handler` обращается к `show_supplier_stock_settings` без префикса). Идентичность объекта функции проверена на трёх путях: через фасад пакета, через `_legacy`, через прямой импорт `supplier_stock` — все три возвращают тот же объект.
- EN: Re-export is two-way: `bot/handlers/settings_handlers/__init__.py` via `from _legacy import *` collects every name (including the re-export from `supplier_stock`), and `_legacy.py` via `from supplier_stock import *` sucks back everything its internal references need (`settings_callback_handler` calls `show_supplier_stock_settings` without prefix). Object identity is verified on three paths: through the package facade, through `_legacy`, and through a direct `supplier_stock` import — all three return the same function object.
- RU: Извлечение сделано AST-скриптом `/tmp/extract_supplier_stock.py` — границы функций считаются по дереву, выборка по regex `(stock|supplier)` в имени, всего собрано 84 функции в порядке появления. Внешние импортёры (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) **не правлены**.
- EN: Extraction was done by an AST script (`/tmp/extract_supplier_stock.py`) — function boundaries are computed from the tree, selection by `(stock|supplier)` regex in the name, 84 functions captured in source order. External importers (`bot/handlers/__init__.py`, `bot/handlers/callbacks.py`, `bot/menu/handlers.py`, `extensions/backup_monitor/bot_handler.py`) are **not touched**.
- RU: Smoke-тесты расширены: `test_settings_handlers_supplier_stock_split` пин-ит, что три пути импорта (`...settings_handlers.X`, `...settings_handlers._legacy.X`, `...settings_handlers.supplier_stock.X`) возвращают идентичный объект и `__module__` указывает в новый модуль. После PR7b в pytest **50 passed**.
- EN: Smoke tests extended: `test_settings_handlers_supplier_stock_split` pins down that the three import paths (`...settings_handlers.X`, `...settings_handlers._legacy.X`, `...settings_handlers.supplier_stock.X`) return the identical object and `__module__` points at the new module. After PR7b pytest has **50 passed**.
- RU: SemVer patch-бамп до `8.62.56`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.56`, `ANDROID_VERSION_CODE=818`).
- EN: SemVer patch bump to `8.62.56`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.56`, `ANDROID_VERSION_CODE=818`).
...
### Changed
- RU: Декомпозиция `core/monitor_core.py` (PR5b серии оптимизации) — оставшиеся ~34 Telegram-handler-функции вынесены в `core/monitor_parts/telegram_handlers.py`:
  - команды и UI-обработчики управления мониторингом: `monitor_status`, `silent_command`/`silent_status_handler`/`force_silent_handler`/`force_loud_handler`/`auto_mode_handler`, `control_command`/`control_panel_handler`/`toggle_monitoring_handler`/`pause_monitoring_handler`/`resume_monitoring_handler`;
  - ресурсные ветки: `check_resources_handler`, `check_cpu/ram/disk/linux/windows/other/all_resources_handler` и парные UI-helpers `perform_linux_check`/`perform_windows_check`/`perform_other_check`/`perform_full_check` (оставлены рядом с вызывающими handlers — они handler-only по природе);
  - меню и отчёты: `close_menu`, `diagnose_menu_handler`, `daily_report_handler`, `toggle_silent_mode_handler`, `send_morning_report_handler`, `debug_morning_report`, `manual_check_handler`, `resource_history_command`, `resource_page_handler`, `refresh_resources_handler`, `close_resources_handler`.
  `core/monitor_core.py` сжался **с 1457 до 382 строк** (с 2783 до начала декомпозиции — общее снижение ~86%) и стал тонким фасадом: остались лишь shared helpers (`is_server_monitoring_enabled`, `refresh_servers`, `ensure_alerts_config`, `ensure_alert_bot`, `send_alert`, `is_silent_time`, `lazy_import`, `get_web_interface_url`, `perform_manual_check`, `send_check_results`, `get_current_server_status`, `_resource_monitor_enabled`, `debug_proxmox_config`) и re-export из подмодулей. Внешние импортёры (`bot/handlers/*`, `bot/menu/*`, `extensions/web_interface`, `core/monitor.py`, `lib/monitoring_utils.py`, `lib/matrix_commands.py`) **не правлены** — `from core.monitor_core import <handler>` продолжает работать. Smoke-тест расширен: импорт `core.monitor_parts.telegram_handlers`.
- EN: Decomposition of `core/monitor_core.py` (PR5b of the optimization series) — the remaining ~34 Telegram handler functions are extracted into `core/monitor_parts/telegram_handlers.py`:
  - commands and monitoring-control UI handlers: `monitor_status`, `silent_command`/`silent_status_handler`/`force_silent_handler`/`force_loud_handler`/`auto_mode_handler`, `control_command`/`control_panel_handler`/`toggle_monitoring_handler`/`pause_monitoring_handler`/`resume_monitoring_handler`;
  - resource branches: `check_resources_handler`, `check_cpu/ram/disk/linux/windows/other/all_resources_handler` and the paired UI helpers `perform_linux_check`/`perform_windows_check`/`perform_other_check`/`perform_full_check` (kept alongside their handlers — they're handler-only by nature);
  - menus and reports: `close_menu`, `diagnose_menu_handler`, `daily_report_handler`, `toggle_silent_mode_handler`, `send_morning_report_handler`, `debug_morning_report`, `manual_check_handler`, `resource_history_command`, `resource_page_handler`, `refresh_resources_handler`, `close_resources_handler`.
  `core/monitor_core.py` shrunk **from 1457 to 382 lines** (from the original 2783 — ~86% reduction over the decomposition series) and is now a thin facade: only shared helpers (`is_server_monitoring_enabled`, `refresh_servers`, `ensure_alerts_config`, `ensure_alert_bot`, `send_alert`, `is_silent_time`, `lazy_import`, `get_web_interface_url`, `perform_manual_check`, `send_check_results`, `get_current_server_status`, `_resource_monitor_enabled`, `debug_proxmox_config`) and re-exports from the submodules remain. External importers (`bot/handlers/*`, `bot/menu/*`, `extensions/web_interface`, `core/monitor.py`, `lib/monitoring_utils.py`, `lib/matrix_commands.py`) **are not touched** — `from core.monitor_core import <handler>` still works. The smoke test is extended with `core.monitor_parts.telegram_handlers`.
- RU: SemVer patch-бамп до `8.62.51`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.51`, `ANDROID_VERSION_CODE=813`).
- EN: SemVer patch bump to `8.62.51`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.51`, `ANDROID_VERSION_CODE=813`).
...
### Changed
- RU: Подготовка к декомпозиции `core/monitor_core.py` (PR4 серии оптимизации) — все 12 module-level переменных состояния (`bot`, `server_status`, `morning_data`, `monitoring_active`, `last_check_time`, `servers`, `resource_history`, `last_resource_check`, `resource_alerts_sent`, `last_report_date`, `last_collection_schedule_time`, `sent_collection_slots`) вынесены в dataclass `MonitoringState` в новом файле `core/monitor_state.py`. Внутри `core/monitor_core.py` все ссылки заменены на обращения через singleton `state`, операторы `global` для этих имён вычищены. Module-level `__getattr__` обеспечивает обратную совместимость с внешними импортёрами (`from core.monitor_core import server_status` и т. п. через `STATE_FIELDS`-прокси), так что `bot/handlers/callbacks.py`, `bot/handlers/commands.py`, `bot/menu/handlers.py`, `extensions/web_interface/__init__.py`, `core/monitor.py`, `lib/monitoring_utils.py` и `lib/matrix_commands.py` не правлены. Поведение не менялось. Smoke-тест `tests/test_imports.py` расширен импортом `core.monitor_state` и фиксацией структуры полей.
- EN: Preparation for splitting `core/monitor_core.py` (PR4 of the optimization series) — all 12 module-level state variables (`bot`, `server_status`, `morning_data`, `monitoring_active`, `last_check_time`, `servers`, `resource_history`, `last_resource_check`, `resource_alerts_sent`, `last_report_date`, `last_collection_schedule_time`, `sent_collection_slots`) are extracted into the `MonitoringState` dataclass in a new `core/monitor_state.py` file. All references inside `core/monitor_core.py` are rewritten to go through the singleton `state`, and `global` statements for those names are stripped. A module-level `__getattr__` keeps backwards compatibility for external importers (`from core.monitor_core import server_status` and the like via the `STATE_FIELDS` proxy), so `bot/handlers/callbacks.py`, `bot/handlers/commands.py`, `bot/menu/handlers.py`, `extensions/web_interface/__init__.py`, `core/monitor.py`, `lib/monitoring_utils.py` and `lib/matrix_commands.py` were not touched. No behavioural change. `tests/test_imports.py` is extended with a `core.monitor_state` import and a structural check of the state fields.
- RU: SemVer patch-бамп до `8.62.48`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.48`, `ANDROID_VERSION_CODE=810`).
- EN: SemVer patch bump to `8.62.48`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.48`, `ANDROID_VERSION_CODE=810`).
...
### Fixed
- RU: Android-приложение: повторно устранён `java.lang.VerifyError` при запуске — после фикса в `8.62.45` (упаковка 94 колбэков в холдер `MonitoringAppCallbacks`) краш-репортер всё равно прислал ту же ошибку: dex-верификатор ART отклонял `MainActivityKt.MonitoringApp` с `invalid arg count (0) in range invoke`. Дополнительной причиной оказался размер самого тела `@Composable`-функции: прямо в нём объявлялось **134 локальных `var ... by rememberSaveable { mutableStateOf(...) }`** (диалоги, инпуты, флаги развёрнутости плашек и т. п.). Compose-компилятор разворачивает каждую такую строку в полноценный composable-вызов с парой `startReplaceableGroup`/`endReplaceableGroup`, конструкцией внутренней лямбды и установкой делегата свойства, и суммарный байт-код метода `MonitoringApp` переполнял лимит инструкций dex/JVM. Теперь все 134 поля состояния вынесены в обычный (не-`@Composable`) класс-холдер `MonitoringAppState` в новом файле `MonitoringAppState.kt`; в теле `MonitoringApp` они заменены одной строкой `val s = remember { MonitoringAppState() }`, а остальное тело экрана обёрнуто в `with(s) { ... }`. Это значит, что все обращения к полям (`isManagementExpanded`, `showServerAvailabilityDialog`, `zfsSelectedHostName`, …) продолжают работать по тем же именам — через `this`-получатель `with`, без правок ~6000 строк остального кода. Регрессия: значения этих полей теряются при пересоздании активити (раньше их сохранял `rememberSaveable`); для всплывающих диалогов это приемлемо — они в любом случае закрываются при повороте экрана, а ключевые настройки приложения продолжают жить в `AppPreferences` и `MainViewModel`.
- EN: Android app: re-fixed the `java.lang.VerifyError` on launch — after the `8.62.45` fix (bundling 94 callbacks into the `MonitoringAppCallbacks` holder), the crash reporter still sent in the same error: ART's dex verifier rejected `MainActivityKt.MonitoringApp` with `invalid arg count (0) in range invoke`. The additional cause turned out to be the size of the `@Composable` function body itself: it directly declared **134 local `var ... by rememberSaveable { mutableStateOf(...) }`** entries (dialogs, inputs, tile-expansion flags, etc.). The Compose compiler expands every such line into a full composable call with a `startReplaceableGroup`/`endReplaceableGroup` pair, an inner-lambda construction and a property-delegate setup, and the resulting bytecode of the `MonitoringApp` method overflowed dex/JVM instruction limits. All 134 state fields are now moved into a plain (non-`@Composable`) holder class `MonitoringAppState` in a new file `MonitoringAppState.kt`; inside `MonitoringApp` they are replaced with a single line `val s = remember { MonitoringAppState() }`, and the remainder of the screen body is wrapped in `with(s) { ... }`. This means every field access (`isManagementExpanded`, `showServerAvailabilityDialog`, `zfsSelectedHostName`, …) keeps working by the same name — via the `with` receiver, without touching the ~6000 lines of remaining code. Regression: the values of these fields are lost when the activity is recreated (they used to be saved by `rememberSaveable`); for transient dialogs this is acceptable — they close on screen rotation anyway, while the key app settings continue to live in `AppPreferences` and `MainViewModel`.

### Changed
- RU: SemVer patch-бамп до `8.62.46`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.46`, `ANDROID_VERSION_CODE=808`).
- EN: SemVer patch bump to `8.62.46`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.46`, `ANDROID_VERSION_CODE=808`).

## [8.62.45] - 2026-05-22

### Fixed
- RU: Android-приложение: первая попытка устранить `java.lang.VerifyError`, из-за которого ComDone падал сразу при запуске («В приложении произошёл сбой»). Встроенный сборщик отчётов (`8.62.43`–`8.62.44`) позволил снять причину прямо с устройства: dex-верификатор ART отклонял класс `MainActivityKt` целиком — `invalid arg count … in invoke-*-range` при вызове `@Composable`-функции `MonitoringApp`. Корень тогда видели в том, что `MonitoringApp` принимала **94 отдельных колбэка-параметра** (`onTokenChanged`, `onSaveBot`, … `onOpenUpdateUrl`); с каждым релизом их число росло, а Compose-компилятор добавляет к ним служебные параметры (`Composer`, маски `$changed`), и суммарное число аргументов вызова переполнило лимит инструкции `invoke-*-range` Dalvik/ART. В этом релизе все 94 колбэка собраны в один неизменяемый класс-холдер `MonitoringAppCallbacks` (`@Immutable`), а `MonitoringApp` принимает всего три параметра — `state`, `preferences`, `callbacks`. Это снимает ограничение по количеству аргументов с большим запасом, но, как показал последующий краш-репорт от `8.62.45`, не устраняет всей причины — оставшийся объём тела функции тоже выходил за лимит; добивающий фикс приехал в `8.62.46`.
- EN: Android app: first attempt at fixing the `java.lang.VerifyError` that made ComDone crash immediately on launch ("the app has stopped"). The built-in crash reporter (`8.62.43`–`8.62.44`) made it possible to capture the cause directly on the device: ART's dex verifier rejected the entire `MainActivityKt` class — `invalid arg count … in invoke-*-range` when invoking the `@Composable` function `MonitoringApp`. The cause was attributed to `MonitoringApp` taking **94 individual callback parameters** (`onTokenChanged`, `onSaveBot`, … `onOpenUpdateUrl`); their number grew with every release, and the Compose compiler adds its own synthetic parameters (`Composer`, `$changed` masks) on top, so the total argument count of the call overflowed the limit of the Dalvik/ART `invoke-*-range` instruction. In this release all 94 callbacks are bundled into a single immutable holder class `MonitoringAppCallbacks` (`@Immutable`), and `MonitoringApp` takes just three parameters — `state`, `preferences`, `callbacks`. This removes the argument-count limit with a wide margin but, as the follow-up crash report from `8.62.45` showed, it does not eliminate the full root cause — the remaining method-body size was also over the limit; the follow-up fix landed in `8.62.46`.

### Changed
- RU: SemVer patch-бамп до `8.62.45`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.45`, `ANDROID_VERSION_CODE=807`).
- EN: SemVer patch bump to `8.62.45`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.45`, `ANDROID_VERSION_CODE=807`).
...
### Added
- RU: Android-приложение получило встроенный сборщик отчётов о сбоях. Раньше при аварийном завершении приложение показывало только системный диалог Android «В приложении "ComDone" произошёл сбой» — без какой-либо технической информации, поэтому диагностировать падение без подключённого к компьютеру `adb logcat` было невозможно. Теперь новый класс `MonitoringApplication` при старте процесса устанавливает глобальный `Thread.setDefaultUncaughtExceptionHandler` (`CrashReporter.install`): любое необработанное исключение — в том числе брошенное при первой композиции Compose на запуске — сохраняется в `SharedPreferences` (синхронным `commit()`) и в файл `comdone-last-crash.txt` в каталоге приложения, после чего управление передаётся системному обработчику. В отчёт попадают метка времени, версия и тип сборки, модель устройства, версия Android, имя потока и полный stack trace. При следующем запуске `MainActivity.onCreate` обнаруживает сохранённый отчёт и вместо обычного интерфейса показывает минимальный экран `CrashReportScreen` с прокручиваемым текстом сбоя, кнопкой «Поделиться отчётом» (системный share-лист — трассировку можно отправить в Telegram/почту без ПК) и кнопкой «Запустить приложение». Экран намеренно не зависит от тяжёлого графа `MonitoringApp`, поэтому отображается даже при детерминированном падении на старте.
- EN: The Android app gained a built-in crash reporter. Previously a crash only produced Android's generic system dialog "В приложении 'ComDone' произошёл сбой" with no technical detail, making it impossible to diagnose a crash without `adb logcat` and a connected computer. Now the new `MonitoringApplication` class installs a global `Thread.setDefaultUncaughtExceptionHandler` at process start (`CrashReporter.install`): any uncaught exception — including one thrown during the first Compose composition on launch — is persisted to `SharedPreferences` (synchronous `commit()`) and to a `comdone-last-crash.txt` file in the app's directory, then control is handed to the system handler. The report captures a timestamp, version and build type, device model, Android version, thread name and the full stack trace. On the next launch `MainActivity.onCreate` detects the stored report and, instead of the normal UI, shows a minimal `CrashReportScreen` with the scrollable crash text, a "Поделиться отчётом" button (the system share sheet — the trace can be sent to Telegram/email with no PC) and a "Запустить приложение" button. The screen deliberately does not depend on the heavy `MonitoringApp` graph, so it shows even for a deterministic startup crash.

### Changed
- RU: SemVer patch-бамп до `8.62.43`; синхронизированы упоминания версии в заголовках исходников/доков, ссылках на prerelease APK и Android-метаданные (`ANDROID_VERSION_NAME=8.62.43`, `ANDROID_VERSION_CODE=805`).
- EN: SemVer patch bump to `8.62.43`; synchronized version mentions in source/doc headers, prerelease APK links and Android metadata (`ANDROID_VERSION_NAME=8.62.43`, `ANDROID_VERSION_CODE=805`).
