...
### Changed
- RU: Android-приложение: плашка «🧩 Расширения» в настройках — фильтр-плашки «Все», «Включено», «Выключено» теперь не информационные `StatChip`-ы и не отдельный ряд `FilterChip`-ов под полем поиска, а три полноценные кнопки-плитки внутри сводной карточки `Surface(primaryContainer)`. Раньше сверху лежали три статичных `StatChip` с теми же подписями и счётчиками (только для просмотра), а под `OutlinedTextField` поиска шёл дублирующий ряд `FilterChip` «Все · N / Включено · N / Выключено · N» — выглядело как две панели об одном и том же, и пользователь не понимал, что фильтр живёт именно во второй. Теперь верхние плашки сами стали интерактивными: каждая — `Surface` с `clickable`, крупным числом и подписью, выбранная плашка подсвечивается своим контейнером (`secondaryContainer` для «Все», `tertiaryContainer` для «Включено», `errorContainer` для «Выключено»), `tonalElevation = 3.dp` и жирным лейблом, невыбранные — нейтральный `surface` + `tonalElevation = 1.dp`. Дублирующий ряд `FilterChip`-ов под поиском полностью убран. Состояние фильтра, как и раньше, сохраняется через `rememberSaveable` и переживает поворот экрана.
- EN: Android app: the "🧩 Extensions" tile in settings — the "Все", "Включено", "Выключено" filter tiles are no longer info-only `StatChip`s plus a separate `FilterChip` row under the search field. They are now three full filter buttons-as-tiles inside the summary card `Surface(primaryContainer)`. Previously three static `StatChip`s sat at the top showing the same labels and counts (read-only), while a duplicated `FilterChip` row "Все · N / Включено · N / Выключено · N" lived under the search `OutlinedTextField` — it looked like two panels about the same thing and users did not realize the filter lived in the second one. The top tiles are now interactive themselves: each is a `Surface` with `clickable`, a large count and a label; the selected tile is highlighted with its own container (`secondaryContainer` for "Все", `tertiaryContainer` for "Включено", `errorContainer` for "Выключено"), `tonalElevation = 3.dp` and a bold label, while the others use a neutral `surface` + `tonalElevation = 1.dp`. The duplicate `FilterChip` row under the search is removed entirely. Filter state is still persisted via `rememberSaveable` and survives screen rotation.
