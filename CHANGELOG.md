...
- EN: SemVer patch bump to `8.56.77`; synchronized explicit current-version mentions across project files and updated Android metadata to `ANDROID_VERSION_NAME=8.56.77` and `ANDROID_VERSION_CODE=652`.
- RU: Выполнен SemVer patch-бамп до `8.56.77`; синхронизированы явные упоминания текущей версии во всех файлах проекта и обновлены Android-метаданные до `ANDROID_VERSION_NAME=8.56.77` и `ANDROID_VERSION_CODE=652`.
