...
### Fixed
- RU: Android-приложение: устранён `java.lang.VerifyError`, из-за которого ComDone падал сразу при запуске («В приложении произошёл сбой»). Встроенный сборщик отчётов (`8.62.43`–`8.62.44`) позволил снять причину прямо с устройства: dex-верификатор ART отклонял класс `MainActivityKt` целиком — `invalid arg count … in invoke-*-range` при вызове `@Composable`-функции `MonitoringApp`. Корень в том, что `MonitoringApp` принимала **94 отдельных колбэка-параметра** (`onTokenChanged`, `onSaveBot`, … `onOpenUpdateUrl`); с каждым релизом их число росло, а Compose-компилятор добавляет к ним служебные параметры (`Composer`, маски `$changed`), и суммарное число аргументов вызова переполнило лимит инструкции `invoke-*-range` Dalvik/ART (счётчик аргументов — 8-битное поле). Теперь все 94 колбэка собраны в один неизменяемый класс-холдер `MonitoringAppCallbacks` (`@Immutable`), а `MonitoringApp` принимает всего три параметра — `state`, `preferences`, `callbacks`. В начале тела функции холдер разворачивается обратно в локальные `val`-переменные с прежними именами, поэтому остальной код экрана (~6000 строк) не менялся. Это снимает ограничение с большим запасом и исключает повторение проблемы при добавлении новых колбэков.
- EN: Android app: fixed the `java.lang.VerifyError` that made ComDone crash immediately on launch ("the app has stopped"). The built-in crash reporter (`8.62.43`–`8.62.44`) made it possible to capture the cause directly on the device: ART's dex verifier rejected the entire `MainActivityKt` class — `invalid arg count … in invoke-*-range` when invoking the `@Composable` function `MonitoringApp`. The root cause: `MonitoringApp` took **94 individual callback parameters** (`onTokenChanged`, `onSaveBot`, … `onOpenUpdateUrl`); their number grew with every release, and the Compose compiler adds its own synthetic parameters (`Composer`, `$changed` masks) on top, so the total argument count of the call overflowed the limit of the Dalvik/ART `invoke-*-range` instruction (the argument count is an 8-bit field). All 94 callbacks are now bundled into a single immutable holder class `MonitoringAppCallbacks` (`@Immutable`), and `MonitoringApp` takes just three parameters — `state`, `preferences`, `callbacks`. At the start of the function body the holder is unpacked back into local `val`s with the original names, so the rest of the screen code (~6000 lines) is unchanged. This removes the limit with a wide margin and prevents the issue from recurring as new callbacks are added.
