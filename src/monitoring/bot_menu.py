...
from monitoring.lib.logging import debug_log
from monitoring.lib.utils import progress_bar, format_duration
from monitoring.config.db_settings import (
    DEBUG_MODE,
    LOG_DIR,
    DATA_DIR,
    DEBUG_LOG_FILE,
...
from monitoring.modules.targeted_checks import targeted_checks

# Ленивые импорты для настроек
def lazy_import_settings_handler():
    """Ленивая загрузка обработчика настроек"""
    try:
        from monitoring.settings_handlers import settings_callback_handler
...
get_config = lazy_import('monitoring.config.db_settings')
get_chat_ids = lazy_import('monitoring.config.db_settings', 'CHAT_IDS')
get_telegram_token = lazy_import('monitoring.config.db_settings', 'TELEGRAM_TOKEN')

# Ленивые импорты утилит
get_debug_log = lambda: debug_log
get_progress_bar = lambda: progress_bar
get_extension_manager = lazy_import('monitoring.extensions.extension_manager', 'extension_manager')
