...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    try:
        from monitoring.core.task_router import TASK_ROUTES
        parser.add_argument(
            "--check",
...
def run_cli_checks(args: argparse.Namespace) -> tuple[bool, int]:
    ...
    from monitoring.core.task_router import run_task
...
def main(args: argparse.Namespace):
    ...
    # ------------------------------------------------------------------
    try:
        from monitoring.config.db_settings import TELEGRAM_TOKEN, DEBUG_MODE, CHAT_IDS, SILENT_START, SILENT_END
    ...
    if not bot_token or len(bot_token) < 10:
        bot_enabled = False        
        if args.dry_run:
            logger.warning("⚠️ Telegram токен отсутствует, dry-run будет без Telegram")
        else:
            logger.warning("⚠️ Telegram токен отсутствует или некорректен, бот отключён")            

    # Применяем настройки алертов заранее
    if not args.dry_run:
        try:
            from monitoring.lib.alerts import configure_alerts
    ...
    if bot_enabled:
        ...
        try:
            from monitoring.lib.alerts import init_telegram_bot
        ...
        # 4. Команды бота
        # ------------------------------------------------------------------
        from monitoring.bot.handlers import (
        ...
        updater = None
        logger.info("ℹ️ Telegram бот отключён: команды не инициализируются")
    ...
    if not args.dry_run:
        try:
            from monitoring.extensions.extension_manager import extension_manager
    ...
    if not args.dry_run:
        try:
            from monitoring.core.monitor import monitor
            if bot_enabled:
                threading.Thread(
                ...
                logger.info("✅ Основной мониторинг запущен без Telegram")
                monitor_thread.join()
                return
        except Exception as e:
    ...
    # ------------------------------------------------------------------
    if not args.dry_run and bot_enabled:
        try:
