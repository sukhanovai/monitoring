...
class SettingsManager:
    ...
    def init_default_settings(self):
        ...
        default_settings = [
            ...
            (
                ...
                "1800",
                "resources",
                "Интервал проверки ресурсов (секунды)",
                "int",
            ),
            (
                ...
                "1800",
                "resources",
                "Интервал повторных алертов ресурсов (секунды)",
                "int",
            ),
            # Пороги ресурсов
    ...
    def get_all_settings(self, category=None):
        ...
        if category:
            cursor.execute(
                "SELECT key, value, data_type, description FROM settings WHERE category = ? ORDER BY key",
                (category,),
            )
        else:
            cursor.execute(
                "SELECT key, value, data_type, description FROM settings ORDER BY category, key"
            )
    ...
    def get_servers_by_type(self, server_type):
        ...
        cursor.execute(
            "SELECT ip, name FROM servers WHERE type = ? AND enabled = 1", (server_type,)
        )
    ...
    def get_all_servers(self, include_disabled=False):
        ...
        if include_disabled:
            cursor.execute(
                "SELECT ip, name, type, credentials, timeout, enabled FROM servers ORDER BY type, name"
            )
        else:
            cursor.execute(
                "SELECT ip, name, type, credentials, timeout, enabled FROM servers WHERE enabled = 1 ORDER BY type, name"
            )
    ...
    def get_backup_patterns(self):
        ...
        cursor.execute(
            "SELECT pattern_type, pattern, category FROM backup_patterns WHERE enabled = 1"
        )
    ...
    def get_duplicate_ip_hosts(self):
        ...
        return self.get_setting(
            "DUPLICATE_IP_HOSTS", {"95.170.153.118": ["pve-rubicon", "pve2-rubicon"]}
        )
    ...
    def update_windows_credential(
        self, cred_id, username=None, password=None, server_type=None, priority=None, enabled=None
    ):
