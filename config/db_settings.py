...
def get_windows_server_configs() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        win2025_ips = [
            s["ip"] for s in windows_servers if s["ip"] in server_groups.get("windows_2025", [])
        ]
        ...
        # domain серверы
        domain_ips = [
            s["ip"] for s in windows_servers if s["ip"] in server_groups.get("domain_servers", [])
        ]
        ...
        # admin серверы
        admin_ips = [
            s["ip"] for s in windows_servers if s["ip"] in server_groups.get("admin_servers", [])
        ]
        ...
        standard_ips = [
            s["ip"] for s in windows_servers if s["ip"] in server_groups.get("standard_windows", [])
        ]
...
def get_servers_config() -> Dict[str, Dict[str, str]]:
    ...
    try:
        ...
        config = {"windows_servers": {}, "linux_servers": {}, "ping_servers": {}}
...
def load_all_settings() -> None:
    ...
    try:
        ...
        MATRIX_ALLOWED_USER_IDS = get_setting(
            "MATRIX_ALLOWED_USER_IDS", defaults.MATRIX_ALLOWED_USER_IDS
        )
        MATRIX_ALLOWED_ROOM_IDS = get_setting(
            "MATRIX_ALLOWED_ROOM_IDS", defaults.MATRIX_ALLOWED_ROOM_IDS
        )
