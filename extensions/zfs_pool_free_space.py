...
EXTENSION_ID = "zfs_pool_free_space_monitor"
HOSTS_SETTING_KEY = "ZFS_POOL_FREE_SPACE_HOSTS"

_ALERT_SENT_AT: dict[str, datetime] = {}
_ALERT_ACTIVE: dict[str, bool] = {}


def normalize_hosts(raw_value: Any) -> dict[str, dict[str, Any]]:
    ...
    for host_name, host_value in raw_value.items():
        ...
        if isinstance(host_value, dict):
            ...
            threshold = host_value.get("threshold", 15)
        else:
            ip = ""
            enabled = True
            threshold = 15
...
def get_hosts_config() -> dict[str, dict[str, Any]]:
    return normalize_hosts(config_manager.get_setting(HOSTS_SETTING_KEY, {}))


def save_hosts_config(hosts: dict[str, dict[str, Any]]) -> None:
...
def collect_zfs_pool_free_space() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    host_map = get_hosts_config()
    results: list[dict[str, Any]] = []
    errors: list[str] = []

    for host_name, host_cfg in sorted(host_map.items()):
...
def check_zfs_pool_free_space_alerts(send_alert_func, repeat_interval_seconds: int = 1800) -> None:
    if not extension_manager.is_extension_enabled(EXTENSION_ID):
        return
    ...
    if errors:
        debug_log(f"⚠️ ZFS pool free space polling issues: {'; '.join(errors)}")
